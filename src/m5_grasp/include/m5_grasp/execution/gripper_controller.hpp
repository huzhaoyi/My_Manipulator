 ...
 *
 * 打开/闭合：写目标 (JointGL, JointGR) 到 /tmp/gripper_direct.txt，由 m5_hardware 通信线程合并后发 UDP。
 * 稳定判据：运动开始后反馈数值不再变化（连续 N 次采样变化 < 阈值）。
 * 实现 IGripper 接口，供 FSM 使用。
 */
...
public:
  ...
  void setParameters(
    ...
    double stable_threshold_rad = 0.02,
    double stable_velocity_eps_rad_s = 0.01,
    int stable_interval_ms = 50,
    int post_execute_wait_ms = 1000,
    double motion_start_threshold_rad = 0.008,
    int motion_start_timeout_ms = 3000,
    int stable_timeout_ms = 8000);
   ...
   * @return 是否成功
   */
  bool openToWidth(double width = -1.0);
   ...
   * @return 是否成功
   */
  bool closeToWidth(double width = -1.0);
   ...
   */
  bool waitForCompletion(int timeout_ms = 8000) override;
   ...
   * @return 对应的关节角度（弧度）
   */
  double widthToJointAngle(double width);

  /**
   * @brief 由关节角（JointGL）反算夹爪宽度，用于日志区分“反馈宽度”与“目标宽度”
   */
  double jointAngleToWidth(double joint_gl) const;
  ...
  /**
   * @brief 设置 /joint_states 诊断缓存，当 getCurrentJointValues 不可用时用其作为轨迹起点后备
   */
  void setJointStateDiag(std::shared_ptr<JointStateDiag> diag) { joint_state_diag_ = std::move(diag); }

  /**
   * @brief 获取打开宽度
   */
  double getOpenWidth() const { return open_width_; }

  /**
   * @brief 获取闭合宽度
   */
  double getCloseWidth() const { return close_width_; }

  /**
   * @brief 检查夹爪是否已到达打开目标（供 FSM WAIT_GRIPPER 每 tick 判断，仅依据稳定性验证标志）
   */
  bool isOpenToTarget() override;

  /**
   * @brief 检查夹爪是否已到达闭合目标（供 FSM WAIT_GRIPPER 每 tick 判断，仅依据稳定性验证标志）
   */
  bool isClosedToTarget() override;
...
private:
  ...
  double stable_velocity_eps_rad_s_{0.01};  // 保留参数兼容，当前稳定判据仅用“数值不变”
  ...
  /** 闭合通过稳定性验证后置 true，供 isClosedToTarget() 持续返回 true，仅在新发 open/close 时清除 */
  mutable bool last_close_verified_stable_{false};
  /** 打开通过稳定性验证后置 true，供 isOpenToTarget() 持续返回 true，仅在新发 open/close 时清除 */
