...
public:
   ...
   */
  bool executeCartesian(const std::vector<geometry_msgs::msg::Pose>& waypoints, bool use_descend = true);
   ...
   */
  bool planExecuteIKJointTargetImpl(
   ...
   * @brief 使用多项式插值执行关节轨迹
   */
  bool executePolynomialJointTrajectory(
   ...
   * @brief 使用B样条执行关节轨迹
   */
  bool executeBSplineJointTrajectory(
   ...
   * @brief 安全获取当前状态
   */
  moveit::core::RobotStatePtr getCurrentStateSafe(double timeout = 1.0);
...
private:
  ...
  std::function<double(double, double, double, double, moveit::core::RobotStatePtr, double, double)> find_valid_yaw_callback_;
  ...
  std::function<std::vector<geometry_msgs::msg::Quaternion>()> compute_orientation_candidates_callback_;
  std::function<void(const std::string&, const std::string&)> publish_state_callback_;

  // Internal helpers
  bool checkIKOnlyImpl(
