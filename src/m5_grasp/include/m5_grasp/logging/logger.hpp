...
public:
  ...
  LogLevel getGlobalLevel() const { return config_.global_level; }
  ...
  std::string getTaskId() const { return current_task_id_; }
  void clearTaskId() { current_task_id_.clear(); }
...
#define LOG_THROTTLE_IMPL(spdlog_level, module, interval_ms, ...) \
  do { \
    if (m5_grasp::logging::ThrottleController::instance().shouldLog(__FILE__, __LINE__, interval_ms)) { \
      M5_LOG_IMPL(spdlog_level, module, __VA_ARGS__); \
    } \
  } while(0)

#define LOG_THROTTLE_INFO(interval_ms, ...)  LOG_THROTTLE_IMPL(spdlog::level::info, "default", interval_ms, __VA_ARGS__)
#define LOG_THROTTLE_WARN(interval_ms, ...)  LOG_THROTTLE_IMPL(spdlog::level::warn, "default", interval_ms, __VA_ARGS__)
#define LOG_THROTTLE_DEBUG(interval_ms, ...) LOG_THROTTLE_IMPL(spdlog::level::debug, "default", interval_ms, __VA_ARGS__)
...
#define LOG_IF(cond, spdlog_level, ...) \
  do { if (cond) M5_LOG_IMPL(spdlog_level, "default", __VA_ARGS__); } while(0)
