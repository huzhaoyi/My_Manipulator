...
#include "m5_grasp/execution/cartesian_executor.hpp"
...
CartesianExecutor::CartesianExecutor(rclcpp::Node* node,
                                     MoveGroupInterfacePtr move_group,
                                     std::mutex& mutex)
    : node_(node), move_group_interface_(move_group), moveit_mutex_(mutex) {}

void CartesianExecutor::setParameters(double eef_step, double jump_threshold,
...
void CartesianExecutor::setLinearParameters(bool use_linear, double linear_velocity,
...
void CartesianExecutor::setBSplineParameters(bool use_bspline, int bspline_degree,
...
void CartesianExecutor::setSegmentParameters(bool adaptive_segments, int min_segments, int max_segments,
...
void CartesianExecutor::setYawValidationParameters(bool enabled, double tolerance, double search_step) {
  yaw_validation_enabled_ = enabled;
  yaw_tolerance_ = tolerance;
  yaw_search_step_ = search_step;
}

void CartesianExecutor::setCallbacks(TransformCallback transform_cb,
...
void CartesianExecutor::setTrajectoryPlanner(TrajectoryPlanner* planner) {
  trajectory_planner_ = planner;
}

void CartesianExecutor::setFrames(const std::string& planning_frame, const std::string& eef_link) {
  planning_frame_ = planning_frame;
  eef_link_ = eef_link;
}

void CartesianExecutor::setIKTimeout(double timeout) {
  ik_timeout_ = timeout;
}

double CartesianExecutor::computeCartesianPathWithTimeout(
    const std::vector<geometry_msgs::msg::Pose>& waypoints,
    double step, double jump_threshold,
    moveit_msgs::msg::RobotTrajectory& trajectory,
    double timeout_sec, bool allow_retry) {
  // 在锁内准备状态
  {
  ...
  if (status == std::future_status::timeout) {
    LOG_NAMED_WARN("cartesian", "笛卡尔路径计算超时（{:.1f}秒，{}个waypoints）", 
                timeout_sec, waypoints.size());
    ...
    if (allow_retry && cartesian_retry_on_timeout_ && waypoints.size() > 2) {
      ...
      double retry_fraction = computeCartesianPathWithTimeout(
          reduced_waypoints, step, jump_threshold, retry_trajectory, timeout_sec * 0.7, false);

      if (retry_fraction >= 0.0) {
        trajectory = retry_trajectory;
        LOG_NAMED_INFO("cartesian", "笛卡尔路径重试成功，成功率: {:.2f}%%", retry_fraction * 100.0);
...
bool CartesianExecutor::executeCartesian(const std::vector<geometry_msgs::msg::Pose>& waypoints,
  ...
  if (use_bspline_ && trajectory_planner_ && 
    ...
    try {
      ...
      if (bspline_waypoints.empty()) {
        ...
        LOG_NAMED_INFO("cartesian", "B样条生成 {} 个笛卡尔轨迹点", bspline_waypoints.size());
  ...
  if (use_linear_interpolation_ && trajectory_planner_ && waypoints.size() == 2) {
    ...
    if (linear_interpolation_success) {
      LOG_NAMED_INFO("cartesian", "使用 {} 个线性插补点进行笛卡尔路径规划", linear_waypoints.size());
      ...
      if (fraction < 0.0) {
        ...
        LOG_NAMED_WARN("cartesian",
                    "线性插补笛卡尔路径执行失败（成功率: {:.2f}%%，阈值: {:.2f}%%），回退到标准方法",
                    fraction * 100.0, threshold * 100.0);
      }
    }
  }
  ...
  moveit_msgs::msg::RobotTrajectory trajectory;
  double fraction = computeCartesianPathWithTimeout(
      waypoints, eef_step_, jump_threshold_, trajectory, timeout, true);
...
int CartesianExecutor::calculateAdaptiveSegments(const geometry_msgs::msg::PoseStamped& start_pose,
  ...
  LOG_NAMED_INFO("cartesian",
              "[自适应分段] 距离={:.3f} m, 计算分段数={} (范围: {}-{})",
              distance, segments, min_segments_, max_segments_);

  return segments;
}

bool CartesianExecutor::executeSegmentedDescend(const geometry_msgs::msg::PoseStamped& start_pose,
                                                const geometry_msgs::msg::PoseStamped& end_pose,
                                                int num_segments) {
  if (num_segments < 0) {
    num_segments = calculateAdaptiveSegments(start_pose, end_pose);
  }

  LOG_NAMED_INFO("cartesian",
              "[分段下压] 开始分段下压，从 ({:.3f}, {:.3f}, {:.3f}) 到 ({:.3f}, {:.3f}, {:.3f})，分段数: {}",
  ...
  for (int i = 1; i < num_segments; ++i) {
    ...
    if (yaw_validation_enabled_ && find_valid_yaw_callback_) {
      valid_yaw = find_valid_yaw_callback_(waypoint.pose.position.x,
                                           waypoint.pose.position.y,
                                           waypoint.pose.position.z,
                                           target_yaw, yaw_tolerance_, yaw_search_step_);
      if (std::isnan(valid_yaw)) valid_yaw = target_yaw;
  ...
  for (size_t i = 1; i < waypoints_stamped.size(); ++i) {
    ...
    if (!plan_execute_callback_ || !plan_execute_callback_(waypoints_stamped[i])) {
      LOG_NAMED_WARN("cartesian",
                  "[分段下压] 段 {}/{} 执行失败，尝试回滚到起点", i + 1, waypoints_stamped.size());
...
bool CartesianExecutor::executeSegmentedLift(const geometry_msgs::msg::PoseStamped& start_pose,
                                             const geometry_msgs::msg::PoseStamped& end_pose,
                                             int num_segments) {
  LOG_NAMED_INFO("cartesian",
              "[分段提升] 开始分段提升，从 ({:.3f}, {:.3f}, {:.3f}) 到 ({:.3f}, {:.3f}, {:.3f})，分段数: {}",
              start_pose.pose.position.x, start_pose.pose.position.y, start_pose.pose.position.z,
              end_pose.pose.position.x, end_pose.pose.position.y, end_pose.pose.position.z,
              num_segments);
  ...
  if (use_bspline_ && trajectory_planner_ && 
    ...
    for (size_t i = 0; i < waypoints.size(); ++i) {
      ...
      if (transform_callback_ && !transform_callback_(pose_in_planning)) {
        LOG_NAMED_WARN("cartesian",
                    "[分段提升] waypoint #{} 转换失败，fallback到逐段执行", i);
      ...
      if (state_for_bspline) {
        ...
        if (jmg_for_bspline) {
          try {
            ...
            LOG_NAMED_WARN("cartesian",
                        "[分段提升] B样条轨迹生成失败: {}，fallback到逐段执行", e.what());
          }
        }
      }
    }
  ...
  for (size_t i = 1; i < waypoints.size(); ++i) {
    ...
    if (!plan_execute_callback_ || !plan_execute_callback_(waypoints[i])) {
      LOG_NAMED_WARN("cartesian",
                  "[分段提升] 段 {}/{} 执行失败，尝试回滚到起点", i + 1, waypoints.size());
    ...
    LOG_NAMED_INFO("cartesian",
                "[分段提升] 段 {}/{} 执行成功，等待状态稳定...", i + 1, waypoints.size());
