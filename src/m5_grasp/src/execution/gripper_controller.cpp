...
// UDP 反馈文件：m5_hardware 每收到 UDP 反馈即写 (JointGL, JointGR) 弧度，夹爪优先读此文件，不依赖 /joint_states
...
void GripperController::setParameters(
    double open_width,
    double close_width,
    double close_extra,
    int hold_time_ms,
    ...
    double width_min,
    int stable_samples,
    double stable_threshold_rad,
    double stable_velocity_eps_rad_s,
    int stable_interval_ms,
    int post_execute_wait_ms,
    double motion_start_threshold_rad,
    int motion_start_timeout_ms,
    int stable_timeout_ms)
{
...
bool GripperController::isFeedbackStable() {
  if (stable_samples_ < 2) return true;
  std::vector<std::pair<double, double>> samples;
  samples.reserve(static_cast<size_t>(stable_samples_));
  for (int i = 0; i < stable_samples_; ++i) {
    std::vector<double> j = getCurrentJointValues();
    if (j.size() < 2u) return false;
...
bool GripperController::waitForMotionStart() {
  if (motion_start_timeout_ms_ <= 0) return true;
  std::vector<double> prev = getCurrentJointValues();
  if (prev.size() < 2u) return false;
  auto start = std::chrono::steady_clock::now();
  while (true) {
    std::this_thread::sleep_for(std::chrono::milliseconds(stable_interval_ms_));
    std::vector<double> cur = getCurrentJointValues();
    if (cur.size() < 2u) continue;
    ...
    if (d_gl >= motion_start_threshold_rad_ || d_gr >= motion_start_threshold_rad_) {
      LOG_NAMED_INFO("gripper", "夹爪运动已开始（数值变化 d_gl={:.4f} d_gr={:.4f} rad）", d_gl, d_gr);
      return true;
    }
    prev = cur;
    ...
    if (elapsed >= motion_start_timeout_ms_) {
      LOG_NAMED_INFO("gripper", "等待运动开始超时 {} ms（可能已在目标），进入判稳", motion_start_timeout_ms_);
      return false;
    }
  }
}

bool GripperController::waitUntilStable() {
  if (stable_timeout_ms_ <= 0) return isFeedbackStable();
  auto start = std::chrono::steady_clock::now();
  while (true) {
    if (isFeedbackStable()) return true;
    auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::steady_clock::now() - start).count();
    if (elapsed >= stable_timeout_ms_) return false;
    std::this_thread::sleep_for(std::chrono::milliseconds(stable_interval_ms_));
  }
}

double GripperController::widthToJointAngle(double width)
{
  // 线性映射
...
double GripperController::jointAngleToWidth(double joint_gl) const {
  const double width_min = width_min_;
  const double width_max = open_width_;
  const double angle_min = angle_min_;
  const double angle_max = angle_max_;
  if (std::abs(angle_max - angle_min) < 1e-9) return width_min;
  double width = width_min + (joint_gl - angle_min) / (angle_max - angle_min) * (width_max - width_min);
  return std::max(width_min, std::min(width_max, width));
}

std::vector<double> GripperController::getCurrentJointValues()
{
  // 优先直接读 UDP 反馈文件（硬件每收到 UDP 即写），不依赖 /joint_states，避免 MoveGroup “latest state time 0” 报错
  struct stat st;
  ...
  if (joint_state_diag_ && node_) {
    ...
    if (fillJointPositionsFromDiag(joint_state_diag_, grip_names, out, now, 5.0) && out.size() >= 2u) {
      return out;
    }
  }
  ...
  try {
    ...
    for (size_t i = 0; i < names.size(); ++i) {
      if (names[i] == "JointGL") gl = raw[i];
      else if (names[i] == "JointGR") gr = raw[i];
    }
    return {gl, gr};
...
bool GripperController::isOpenToTarget() {
  // 保持粘性：验证通过后持续返回 true，供 FSM 在 min_dwell 满足后转 PREGRASP；仅在新发 open/close 时清除
  return last_open_verified_stable_;
}

bool GripperController::isClosedToTarget() {
  // 保持粘性：验证通过后持续返回 true，供 FSM 在 min_dwell 满足后转 LIFT；仅在新发 open/close 时清除
  return last_close_verified_stable_;
}

bool GripperController::open()
{
  return openToWidth(-1.0);
}

bool GripperController::close()
{
  return closeToWidth(-1.0);
}

bool GripperController::waitForCompletion(int timeout_ms)
{
  // FSM 已使用非阻塞 WAIT_GRIPPER，此处不再阻塞，仅保留接口兼容
  (void)timeout_ms;
  return true;
}

bool GripperController::openToWidth(double width)
{
  ...
  double joint_gl = widthToJointAngle(width);
  double joint_gr = -joint_gl;  // 镜像对称
  {
    std::ofstream f(GRIPPER_DIRECT_FILE);
    if (!f) {
      LOG_NAMED_WARN("gripper", "[夹爪打开] 无法写入 {}，夹爪直接目标未下发", GRIPPER_DIRECT_FILE);
      return false;
    }
    f << joint_gl << " " << joint_gr << "\n";
  }
  LOG_NAMED_INFO("gripper", "[夹爪打开] 已写目标 JointGL={:.4f} rad, JointGR={:.4f} rad（0=闭合 -1100=全开，直接 UDP）", joint_gl, joint_gr);

  if (post_execute_wait_ms_ > 0)
    std::this_thread::sleep_for(std::chrono::milliseconds(post_execute_wait_ms_));
  waitForMotionStart();  // 先等“数值在变化”（运动开始），超时也继续
  bool stable = waitUntilStable();  // 再等“数值不变”=到位
  if (!stable)
    LOG_NAMED_WARN("gripper", "夹爪打开判稳超时，按成功继续");
  last_open_verified_stable_ = true;
  LOG_NAMED_INFO("gripper", "夹爪打开完成（{}），宽度: {:.3f} m", stable ? "反馈已稳定" : "超时视为通过", width);
  return true;
}

bool GripperController::closeToWidth(double width)
{
  last_close_verified_stable_ = false;

  if (width < 0.0) {
    width = close_width_ - close_extra_;
    LOG_NAMED_INFO("gripper", "[夹爪闭合] 使用配置参数: close_width={:.3f} m, close_extra={:.3f} m, 最终宽度={:.3f} m",
  ...
  double joint_gr = -joint_gl;  // 镜像对称
  {
    std::ofstream f(GRIPPER_DIRECT_FILE);
    if (!f) {
      LOG_NAMED_WARN("gripper", "[夹爪闭合] 无法写入 {}，夹爪直接目标未下发", GRIPPER_DIRECT_FILE);
      return false;
    }
    f << joint_gl << " " << joint_gr << "\n";
  }
  LOG_NAMED_INFO("gripper", "[夹爪闭合] 已写目标 JointGL={:.4f} rad, JointGR={:.4f} rad（0=闭合，直接 UDP）", joint_gl, joint_gr);

  if (post_execute_wait_ms_ > 0)
    std::this_thread::sleep_for(std::chrono::milliseconds(post_execute_wait_ms_));
  waitForMotionStart();  // 先等“数值在变化”（运动开始）
  if (!waitUntilStable()) {
    LOG_NAMED_WARN("gripper", "夹爪闭合判稳超时（数值未不变=未夹到或未到位），按失败处理");
    return false;
  }
  last_close_verified_stable_ = true;
  std::vector<double> fb = getCurrentJointValues();
  const double target_width = (width < 0.0) ? (close_width_ - close_extra_) : width;
  if (fb.size() >= 2u) {
    double fb_width = jointAngleToWidth(fb[0]);
    LOG_NAMED_INFO("gripper", "夹爪闭合成功（数值不变=夹到/到位），反馈宽度 {:.4f} m，目标 {:.4f} m", fb_width, target_width);
  } else {
