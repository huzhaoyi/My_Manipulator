...
#include "m5_grasp/execution/motion_executor.hpp"
...
MotionExecutor::MotionExecutor(
  ...
  , trajectory_planner_(trajectory_planner)
...
void MotionExecutor::setParameters(
    ...
    const std::string& polynomial_type,
...
void MotionExecutor::setFindValidYawCallback(
    std::function<double(double, double, double, double, moveit::core::RobotStatePtr, double, double)> callback)
{
...
moveit::core::RobotStatePtr MotionExecutor::getCurrentStateSafe(double timeout)
{
  ...
  try {
    ...
    LOG_NAMED_WARN("motion", "getCurrentStateSafe异常: {}", e.what());
  }
...
std::vector<double> MotionExecutor::getCurrentJointValuesSafe()
{
  std::lock_guard<std::mutex> lock(moveit_mutex_);
  return move_group_interface_->getCurrentJointValues();
}

geometry_msgs::msg::PoseStamped MotionExecutor::getCurrentPoseSafe()
{
  std::lock_guard<std::mutex> lock(moveit_mutex_);
...
bool MotionExecutor::waitForStateStable(double timeout_sec, int check_count, double threshold)
{
  ...
  while (std::chrono::steady_clock::now() - start_time < timeout_duration) {
    std::vector<double> current_joints = getCurrentJointValuesSafe();
    ...
    bool allow_retry)
{
  ...
  if (status == std::future_status::timeout) {
    LOG_NAMED_WARN("motion", "笛卡尔路径计算超时（{:.1f}秒，{}个waypoints）", 
...
bool MotionExecutor::executeCartesian(const std::vector<geometry_msgs::msg::Pose>& waypoints, bool use_descend)
{
  ...
  if (use_linear_interpolation_ && waypoints.size() == 2) {
    ...
    try {
      std::vector<geometry_msgs::msg::Pose> linear_waypoints = 
          trajectory_planner_.generateLinearCartesianTrajectory(
...
int MotionExecutor::calculateAdaptiveSegments(
...
bool MotionExecutor::executeSegmentedDescend(
    const geometry_msgs::msg::PoseStamped& start_pose,
    const geometry_msgs::msg::PoseStamped& end_pose,
    int num_segments)
{
...
bool MotionExecutor::executeSegmentedLift(
    const geometry_msgs::msg::PoseStamped& start_pose,
    const geometry_msgs::msg::PoseStamped& end_pose,
    int num_segments)
{
...
bool MotionExecutor::checkIKOnlyImpl(
    const geometry_msgs::msg::PoseStamped& pose_in_planning,
...
bool MotionExecutor::planExecuteIKJointTarget(
    const geometry_msgs::msg::PoseStamped& target_pose,
    int max_ik_attempts)
{
  std::lock_guard<std::mutex> lock(moveit_mutex_);
...
bool MotionExecutor::executePolynomialJointTrajectory(
...
bool MotionExecutor::executeWithRecovery(
    moveit::planning_interface::MoveGroupInterface::Plan& plan,
