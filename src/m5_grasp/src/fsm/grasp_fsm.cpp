...
const char* graspStateToString(GraspState state) {
  switch (state) {
    case GraspState::IDLE: return "IDLE";
    case GraspState::WAIT_TARGET_STABLE: return "WAIT_TARGET_STABLE";
    case GraspState::OPEN_GRIPPER: return "OPEN_GRIPPER";
    case GraspState::WAIT_GRIPPER: return "WAIT_GRIPPER";
    case GraspState::PREGRASP: return "PREGRASP";
    case GraspState::WAIT_EXECUTION_RESULT: return "WAIT_EXECUTION_RESULT";
    case GraspState::WAIT_ARM_STABLE: return "WAIT_ARM_STABLE";
    case GraspState::ALIGN: return "ALIGN";
    case GraspState::DESCEND: return "DESCEND";
    case GraspState::CLOSE_GRIPPER: return "CLOSE_GRIPPER";
    case GraspState::LIFT: return "LIFT";
    case GraspState::DONE: return "DONE";
    case GraspState::ABORT_SAFE: return "ABORT_SAFE";
    case GraspState::RETRY_BACKOFF: return "RETRY_BACKOFF";
...
void GraspFSM::setCurrentTarget(const TaskTarget& target) {
  *current_target_ = target;
}

void GraspFSM::triggerWaitTarget() {
  GraspState current = state_.load();
  if (current == GraspState::IDLE) {
    transitionTo(GraspState::WAIT_TARGET_STABLE);
  }
}

void GraspFSM::triggerStartGrasp() {
  if (state_.load() == GraspState::IDLE) {
    transitionTo(GraspState::OPEN_GRIPPER);
  }
}

void GraspFSM::setConfig(const Config& config) {
  config_ = config;
}

void GraspFSM::setStateChangeCallback(StateChangeCallback callback) {
...
void GraspFSM::transitionTo(GraspState new_state) {
  ...
  if (old_state != new_state) {
    LOG_NAMED_INFO("grasp_fsm", "[FSM] 状态转换: {} -> {}", 
                  graspStateToString(old_state), graspStateToString(new_state));
...
void GraspFSM::doAbortSafe(const rclcpp::Time& now) {
  ...
  if (gripper_) {
    ...
    if (task_runner_) {
      task_runner_->setEmergencyStop(false);
    }
...
void GraspFSM::tick(const rclcpp::Time& now) {
  // 检查急停：非 IDLE/ABORT_SAFE 时先转 ABORT_SAFE；已在 ABORT_SAFE 时继续执行 handleAbortSafe 以便清除急停并回到 IDLE
  if (emergency_stop_.load()) {
    ...
    // current == ABORT_SAFE：不 return，继续到 switch 中执行 handleAbortSafe，完成后清除急停并回到 IDLE
  }
  ...
  switch (current) {
    ...
    case GraspState::WAIT_TARGET_STABLE:
      // 已废弃：不再等待稳定，由 triggerStartGrasp 直接进入 OPEN_GRIPPER
      break;
...
void GraspFSM::handleWaitTargetStable(const rclcpp::Time& now) {
  if (tracker_->hasStableTarget(now)) {
    auto target_opt = tracker_->getStableTarget();
    if (target_opt) {
      // 计算任务目标
      // 注意：这里需要TaskContext来计算，但FSM不应该持有它
      // 方案：让外部（Node）负责计算target并注入
      // 暂时跳过计算，假设target已经是有效的
      
      plan_retry_count_.store(0);
      exec_retry_count_.store(0);
      transitionTo(GraspState::OPEN_GRIPPER);
      LOG_NAMED_INFO("grasp_fsm", "[FSM] 目标稳定，进入OPEN_GRIPPER");
    }
  }
}
...
void GraspFSM::handleWaitGripper(const rclcpp::Time& now) {
  if (gripper_) {
    // 仅“正常打开→预抓”时夹爪打开后进 PREGRASP；需满足到位且打开最小停留，避免臂动时夹爪仍在张开
    if (wait_gripper_open_ && !wait_gripper_abort_ && gripper_->isOpenToTarget()) {
      ...
      double open_min_dwell_s = config_.gripper_open_min_dwell_ms > 0
          ? (config_.gripper_open_min_dwell_ms / 1000.0) : 0.0;
    ...
    if (!wait_gripper_open_ && !wait_gripper_abort_ && gripper_->isClosedToTarget()) {
      ...
      double min_dwell_s = config_.gripper_close_min_dwell_ms > 0
          ? (config_.gripper_close_min_dwell_ms / 1000.0) : 0.0;
  ...
  if (elapsed_s >= timeout_s) {
    if (wait_gripper_abort_) {
      ...
      // 闭合等待超时：未在超时内得到“闭合验证通过”或未满足 min_dwell（到位/抓取成功均由 gripper 稳定判定）
      transitionTo(GraspState::ABORT_SAFE);
      LOG_NAMED_WARN("grasp_fsm", "[FSM] 夹爪闭合等待超时({:.1f}s)，未在超时内满足闭合验证+min_dwell，进入ABORT_SAFE", elapsed_s);
    }
  }
...
void GraspFSM::handlePregrasp(const rclcpp::Time& /*now*/) {
  // 注意：目标已在 WAIT_TARGET_STABLE 阶段锁定
  // 执行阶段不再检查过期/跳变，避免因夹爪操作延时导致循环
  ...
  if (task_runner_->runPregrasp(*current_target_)) {
    // 已启动异步执行，进入等待状态
...
void GraspFSM::handleWaitExecutionResult(const rclcpp::Time& /*now*/) {
  ...
  int status = task_runner->checkExecutionStatus();
  
  // 根据任务名称决定下一步转移
  std::string task_name = task_runner->getCurrentTaskName();
  ...
  if (task_name == "pregrasp") {
    ...
    LOG_NAMED_WARN("grasp_fsm", "[FSM] WAIT_EXECUTION_RESULT: 未知任务名称 '{}'，使用默认逻辑", task_name);
    next_state = GraspState::ALIGN;
    fallback_state = GraspState::IDLE;
  }
  ...
  if (status == 2) {  // 成功
    ...
    task_runner->clearExecutionState();
    // 机械臂每步完成后非阻塞等待关节稳定再进入下一步
    if (config_.arm_stable_timeout_ms > 0 && config_.arm_stable_window > 0) {
      next_state_after_stable_ = next_state;
      task_runner->startArmStableCheck(
          config_.arm_stable_timeout_ms,
          config_.arm_stable_threshold_rad,
          config_.arm_stable_window,
          config_.arm_stable_velocity_eps_rad_s,
          config_.arm_stable_min_dwell_ms);
      transitionTo(GraspState::WAIT_ARM_STABLE);
      LOG_NAMED_INFO("grasp_fsm", "[FSM] 任务 '{}' 执行成功，进入WAIT_ARM_STABLE（非阻塞）", task_name);
    } else {
      transitionTo(next_state);
      LOG_NAMED_INFO("grasp_fsm", "[FSM] 任务 '{}' 执行成功，进入{}", task_name, graspStateToString(next_state));
    }
  } else if (status == 3 || status == 4) {  // 失败或超时
    LOG_NAMED_WARN("grasp_fsm", "[FSM] 任务 '{}' 执行{}（status={}）", 
                   task_name, status == 3 ? "失败" : "超时", status);
    // 清理执行状态
    task_runner->clearExecutionState();
...
void GraspFSM::handleWaitArmStable(const rclcpp::Time& now) {
  ...
  auto [done, stable] = task_runner->tickArmStableCheck(now);
  if (!done) {
    return;
  }
  if (!stable) {
    // 超时 = 未达到稳定阈值，禁止继续下一步，避免“流程结束但实机还在动”
    LOG_NAMED_ERROR("grasp_fsm", "[FSM] 关节稳定等待超时，动作未稳定，禁止继续（进入 ABORT_SAFE）");
    transitionTo(GraspState::ABORT_SAFE);
    return;
  }
  transitionTo(next_state_after_stable_);
  LOG_NAMED_INFO("grasp_fsm", "[FSM] 进入{}", graspStateToString(next_state_after_stable_));
}
...
void GraspFSM::handleAlign(const rclcpp::Time& /*now*/) {
  ...
  if (task_runner_->runAlign(*current_target_)) {
    // 已启动异步执行，进入等待状态
    transitionTo(GraspState::WAIT_EXECUTION_RESULT);
...
void GraspFSM::handleDescend(const rclcpp::Time& /*now*/) {
  ...
  if (task_runner_->runDescend(*current_target_)) {
    // 已启动异步执行，进入等待状态
    transitionTo(GraspState::WAIT_EXECUTION_RESULT);
...
void GraspFSM::handleLift() {
  ...
  if (task_runner_->runLift(*current_target_)) {
    // 已启动异步执行，进入等待状态
    transitionTo(GraspState::WAIT_EXECUTION_RESULT);
...
void GraspFSM::handleAbortSafe(const rclcpp::Time& now) {
  doAbortSafe(now);
  // 若有夹爪则已转入 WAIT_GRIPPER，清理与回 IDLE 在 handleWaitGripper 超时后做；无夹爪时 doAbortSafe 内部已清理并转 IDLE
}

void GraspFSM::emergencyStop() {
...
GraspState GraspFSM::getCurrentState() const {
  return state_.load();
}

std::string GraspFSM::getCurrentStateString() const {
  return graspStateToString(state_.load());
}

bool GraspFSM::isExecuting() const {
  GraspState current = state_.load();
