...
bool TargetTracker::hasStableTarget(const rclcpp::Time& now) const {
  ...
  if (window_.size() < config_.window_size) {
    LOG_NAMED_DEBUG("target_tracker", "目标不稳定: 滑窗帧数不足 ({}/{})", 
                   window_.size(), config_.window_size);
    return false;
  }
  ...
  if (age > config_.stale_timeout) {
    LOG_NAMED_DEBUG("target_tracker", "目标不稳定: 目标过期 ({:.2f}s > {:.2f}s)", 
                   age, config_.stale_timeout);
    return false;
  }
  ...
  for (const auto& t : window_) {
    ...
    double dyaw = std::abs(normalizeAngle(t.yaw - base.yaw));
    max_yaw = std::max(max_yaw, dyaw);
  }
  
  // 阈值判断
  if (max_d >= config_.stable_pos_threshold || max_yaw >= config_.stable_yaw_threshold) {
    LOG_NAMED_DEBUG("target_tracker", "目标不稳定: max_d={:.3f}m (阈值{:.3f}), max_yaw={:.1f}° (阈值{:.1f}°)", 
                   max_d, config_.stable_pos_threshold, 
                   max_yaw * 180.0 / M_PI, config_.stable_yaw_threshold * 180.0 / M_PI);
    return false;
  }
  
  LOG_NAMED_INFO("target_tracker", "目标稳定: max_d={:.3f}m, max_yaw={:.1f}°", 
                max_d, max_yaw * 180.0 / M_PI);
  return true;
}
...
bool TargetTracker::isJumping(const m5_msgs::msg::CablePoseWithYaw& ref) const {
  ...
  // 计算yaw偏移
  double yaw_jump = std::abs(normalizeAngle(latest_target_.yaw - ref.yaw));
  
  if (pos_jump >= config_.jump_pos_threshold || yaw_jump >= config_.jump_yaw_threshold) {
    LOG_NAMED_WARN("target_tracker", "目标跳变: pos={:.3f}m (阈值{:.3f}), yaw={:.1f}° (阈值{:.1f}°)", 
                  pos_jump, config_.jump_pos_threshold, 
                  yaw_jump * 180.0 / M_PI, config_.jump_yaw_threshold * 180.0 / M_PI);
    return true;
  }
...
bool TargetTracker::isValid(const rclcpp::Time& now, const m5_msgs::msg::CablePoseWithYaw& ref) const {
  if (isStale(now)) {
...
double TargetTracker::normalizeAngle(double angle) {
