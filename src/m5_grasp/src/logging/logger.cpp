...
void crashSignalHandler(int sig) {
  ...
  // dump ring buffer
  std::string crash_file = "/tmp/m5_grasp_crash_" + 
...
void LogManager::init(const LogConfig& config) {
  ...
  try {
    ...
    LOG_INFO("日志系统初始化完成: level={}, async={}, file={}, console={}",
             levelToString(config_.global_level),
             config_.async_mode ? "true" : "false",
...
void LogManager::setupSinks() {
  ...
  if (config_.file_enabled) {
    ...
    if (config_.daily_rotation) {
      ...
      auto file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
          log_file, 
          config_.max_file_size_mb * 1024 * 1024,
          config_.max_files);
      file_sink->set_level(toSpdlogLevel(config_.global_level));
      sinks.push_back(file_sink);
    }
  ...
  if (config_.ring_buffer_enabled) {
    auto ring_sink = std::make_shared<spdlog::sinks::ringbuffer_sink_mt>(config_.ring_buffer_size);
