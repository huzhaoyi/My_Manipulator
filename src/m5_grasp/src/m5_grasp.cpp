...
class M5Grasp : public rclcpp::Node
{
  public:
    ...
    void start_executor()
    {
        ...
        cable_pose_sub_ = this->create_subscription<m5_msgs::msg::CablePoseWithYaw>(
            "/cable_pose_with_yaw", 10,
            std::bind(&M5Grasp::cable_pose_callback, this, std::placeholders::_1), sub_options);

        emergency_stop_sub_ = this->create_subscription<std_msgs::msg::String>(
            "/emergency_stop", 10,
            std::bind(&M5Grasp::emergency_stop_callback, this, std::placeholders::_1), sub_options);
        ...
        executor_thread_ = std::thread(
            [this]()
            {
                LOG_NAMED_INFO("m5_grasp", "Executor线程启动（MultiThreadedExecutor，4线程，callbac"
                                           "k group分离：快速/慢速）");
                int spin_count = 0;
    ...
    void init_move_it()
    {
        ...
        if (!param_manager_->add_ground_plane && planning_scene_)
        {
            ...
            LOG_NAMED_INFO("m5_grasp", "已移除地面碰撞体 ground_plane（支架测试模式）");
        }

        // 初始化夹爪控制器
        gripper_controller_ =
            std::make_shared<m5_grasp::GripperController>(this, gripper_group_, moveit_mutex_);
        ...
        if (executor_ && cb_fast_)
        {
            try
            {
                ...
                LOG_NAMED_INFO("m5_grasp", "✓ 已手动将 callback_group_fast 添加到 executor（确保 "
        ...
        grasp_fsm_ = std::make_unique<m5_grasp::GraspFSM>(task_runner_, gripper_controller_);
        ...
        grasp_fsm_->set_state_change_callback(
            [this](m5_grasp::GraspState old_state, m5_grasp::GraspState new_state)
            {
                ...
                last_display_state_str_ = m5_grasp::grasp_state_to_string(display_state);
        ...
        fsm_timer_ = this->create_wall_timer(std::chrono::milliseconds(100),
                                             std::bind(&M5Grasp::fsm_tick, this),
  ...
  private:
    ...
    void init_task_context()
    {
        ...
        LOG_NAMED_INFO("m5_grasp", "主 node 指针地址: node_ptr={}, node_base_ptr={}", main_node_ptr,
                       main_node_base_ptr);
     ...
     */
    void cable_pose_callback(const m5_msgs::msg::CablePoseWithYaw::SharedPtr msg)
    {
        // 检查是否已初始化
    ...
    void fsm_tick()
    {
        ...
        if (grasp_fsm_->get_current_state() == m5_grasp::GraspState::IDLE && pending_start_grasp_ &&
            is_joint_states_ready())
        {
            grasp_fsm_->set_current_target(pending_target_);
            grasp_fsm_->trigger_start_grasp();
            pending_start_grasp_ = false;
            LOG_NAMED_INFO("m5_grasp", "joint_states 已就绪，开始抓取");
        }
    ...
    m5_grasp::TaskTarget pending_target_;        // 待启动时的目标
