...
TaskFactory::TaskFactory(const TaskContext& ctx)
  : ctx_(ctx)
{
  // 提前创建专用 MTC 规划 node，名称带唯一后缀，减少同名 node/logger 导致的 "Publisher already registered"
  if (ctx_.node) {
    int id = g_mtc_planning_node_counter++;
    std::string name = std::string(ctx_.node->get_name()) + "_mtc_planning_" + std::to_string(id);
    mtc_planning_node_ = rclcpp::Node::make_shared(name);
    LOG_NAMED_INFO("task_factory", "TaskFactory: 已创建专用 MTC 规划 node: {}", name);
  }
}

std::shared_ptr<mtc::solvers::PipelinePlanner> TaskFactory::createOMPLPlanner() {
  // 使用构造时已创建的专用 node（名称唯一）调用 PipelinePlanner::create，避免与主节点同名导致 "Publisher already registered"
  if (!cached_ompl_planner_) {
    if (!mtc_planning_node_) {
      LOG_NAMED_ERROR("task_factory", "mtc_planning_node_ 未初始化，无法创建 PipelinePlanner");
      return nullptr;
    }
    if (!ctx_.move_group) {
    ...
    try {
      ...
      LOG_NAMED_INFO("task_factory", "创建 PipelinePlanner: 使用已有 PlanningPipeline（无 node 持有）");
...
std::shared_ptr<mtc::solvers::CartesianPath> TaskFactory::createCartesianPlanner() {
  ...
  if (!cached_cartesian_planner_) {
    ...
    cached_cartesian_planner_->setMaxAccelerationScalingFactor(ctx_.cartesian_acceleration_scaling);
    cached_cartesian_planner_->setStepSize(ctx_.cartesian_step_size);
  }
  return cached_cartesian_planner_;
}

void TaskFactory::addCurrentStateStage(mtc::Task& task) {
  if (!ctx_.move_group) {
  ...
  // planning scene 由 /joint_states 更新（机械臂反馈 → parse_feedback_json → joint_state_broadcaster → /joint_states）。
...
void TaskFactory::addStartStateStageForCartesian(mtc::Task& task) {
  if (!ctx_.move_group) {
    addCurrentStateStage(task);
    return;
  }
  const auto& robot_model = ctx_.move_group->getRobotModel();
  if (!robot_model) {
  ...
  // 若 joint_state_diag 可用且能填满 arm，用 FixedState 注入起始状态，避免 task.plan() 在 CurrentState 上无限等待 planning scene。
  // 新鲜度 5s：与 cleanupMoveGroupState / 夹爪一致，从收到目标到执行有数秒延迟
  constexpr double kDiagMaxAgeSec = 5.0;
  if (ctx_.joint_state_diag && ctx_.node) {
    ...
    if (arm_jmg) {
      ...
      if (fillJointPositionsFromDiag(ctx_.joint_state_diag,
                                     arm_jmg->getActiveJointModelNames(),
                                     arm_vals,
                                     now,
                                     kDiagMaxAgeSec)) {
        ...
        LOG_NAMED_INFO("task_factory", "使用 FixedState（joint_state_diag），避免 planning scene 阻塞");
        return;
      }
    }
  }
...
std::unique_ptr<mtc::stages::MoveTo> TaskFactory::makeMoveToPose(
...
std::unique_ptr<mtc::stages::MoveTo> TaskFactory::makeMoveToJoints(
    const std::string& name,
...
std::unique_ptr<mtc::stages::MoveRelative> TaskFactory::makeMoveRelativeZ(
    const std::string& name,
    double distance,
...
mtc::Task TaskFactory::makePregrasp(const TaskTarget& target) {
  LOG_NAMED_INFO("task_factory", "[makePregrasp] 构建预抓取任务");
  ...
  if (!ctx_.move_group) {
    LOG_NAMED_ERROR("task_factory", "[makePregrasp] move_group 无效，无法构建任务");
    return task;  // 返回空任务
  }
  
  const auto& robot_model = ctx_.move_group->getRobotModel();
  if (!robot_model) {
    LOG_NAMED_ERROR("task_factory", "[makePregrasp] 无法获取 robot model，无法构建任务");
    return task;  // 返回空任务
  }
  // 说明：task.setRobotModel() 或后续 task.init() 可能触发 MTC 内部创建与主节点同名的 logger，产生 "Publisher already registered" 警告，非致命
  task.setRobotModel(robot_model);
  LOG_NAMED_INFO("task_factory", "[makePregrasp] setRobotModel 完成");
  task.stages()->setName("pregrasp");
  LOG_NAMED_INFO("task_factory", "[makePregrasp] stages()->setName 完成");
  // 关键修复：禁用 introspection 避免创建新的 publisher，导致 "Publisher already registered" 警告
  task.enableIntrospection(false);
  LOG_NAMED_INFO("task_factory", "[makePregrasp] enableIntrospection(false) 完成");
  
  // 关节目标用 JointInterpolationPlanner，不创建 PlanningPipeline，避免 "Publisher already registered" / SIGABRT
  ...
  if (target.use_joint_goal && !target.pregrasp_joint_map.empty()) {
    if (!cached_joint_planner_) {
      ...
      LOG_NAMED_INFO("task_factory", "[makePregrasp] 使用 JointInterpolationPlanner（关节目标，无 node）");
    }
    planner = cached_joint_planner_;
  } else {
    planner = createOMPLPlanner();
    if (!planner) {
      LOG_NAMED_ERROR("task_factory", "[makePregrasp] PipelinePlanner 创建失败，无法构建任务");
      return task;
    }
  }
  
  // 1. 添加起始状态：优先 FixedState（diag），避免 MTC CurrentState 从 planning scene 取到无效状态导致 OMPL "Unable to sample any valid states for goal tree"
  LOG_NAMED_INFO("task_factory", "[makePregrasp] 即将 addStartStateStageForCartesian（diag 可用则 FixedState）");
  addStartStateStageForCartesian(task);
  LOG_NAMED_INFO("task_factory", "[makePregrasp] addStartStateStageForCartesian 返回");
  
  // 2. 移动到预抓取位置
  // 注意：碰撞对象在pregrasp之后添加，避免goal state被判碰撞
  LOG_NAMED_INFO("task_factory", "[makePregrasp] 即将 task.add(move to pregrasp)");
  if (target.use_joint_goal && !target.pregrasp_joint_map.empty()) {
    task.add(makeMoveToJoints("move to pregrasp (joint)", target.pregrasp_joint_map, planner));
    LOG_NAMED_INFO("task_factory", "[makePregrasp] 使用关节目标模式");
  } else {
    task.add(makeMoveToPose("move to pregrasp (pose)", target.pregrasp_pose, planner));
    LOG_NAMED_INFO("task_factory", "[makePregrasp] 使用位姿目标模式");
  }
  LOG_NAMED_INFO("task_factory", "[makePregrasp] task.add(move to pregrasp) 返回");
  
  // 3. 添加碰撞对象（在pregrasp之后，避免goal state被判碰撞）
  LOG_NAMED_INFO("task_factory", "[makePregrasp] 即将 task.add(makeAddCollisionObject)");
  task.add(makeAddCollisionObject(target));
  LOG_NAMED_INFO("task_factory", "[makePregrasp] task.add(makeAddCollisionObject) 返回");
  
  // 4. 设置ACM允许夹爪与缆绳碰撞（对于后续的descend/close阶段至关重要）
  LOG_NAMED_INFO("task_factory", "[makePregrasp] 即将 task.add(makeAllowCollisions)");
  task.add(makeAllowCollisions());
  LOG_NAMED_INFO("task_factory", "[makePregrasp] task.add(makeAllowCollisions) 返回，makePregrasp 完成");
  
  return task;
}

mtc::Task TaskFactory::makeAlign(const TaskTarget& target) {
  ...
  // align 仅设 Joint4，本质为单关节目标，与 pregrasp/descend 一致使用 JointInterpolationPlanner，避免 CHOMP 碰撞/无运动轨迹问题
  std::shared_ptr<mtc::solvers::PlannerInterface> planner;
  if (!cached_joint_planner_) {
    cached_joint_planner_ = std::make_shared<mtc::solvers::JointInterpolationPlanner>();
    LOG_NAMED_INFO("task_factory", "[makeAlign] 使用 JointInterpolationPlanner（关节目标，无 node）");
  }
  planner = cached_joint_planner_;
  
  // 1. 添加起始状态：优先 FixedState（diag），与 makePregrasp 一致
  addStartStateStageForCartesian(task);

  // 2. ACM 允许臂与缆绳碰撞（与 makeDescend 一致），避免 CHOMP 在仅动 Joint4 时判臂-缆绳碰撞失败
  ...
  if (arm_link_names.empty()) {
    LOG_NAMED_WARN("task_factory", "[makeAlign] arm_group '{}' 未找到或无 link，尝试备用：用非夹爪 link 设置臂-缆绳 ACM",
                   ctx_.arm_group);
    const std::vector<std::string>& all_names = robot_model->getLinkModelNames();
    std::set<std::string> gripper_set(ctx_.allow_touch_links.begin(), ctx_.allow_touch_links.end());
    for (const std::string& link_name : all_names) {
      if (gripper_set.count(link_name)) continue;
      arm_link_names.push_back(link_name);
    }
    if (!arm_link_names.empty()) {
      LOG_NAMED_INFO("task_factory", "[makeAlign] 备用: 为 {} 个非夹爪 link 设置臂-缆绳 ACM", arm_link_names.size());
    }
  }
  if (!arm_link_names.empty()) {
    auto allow_arm = std::make_unique<mtc::stages::ModifyPlanningScene>("allow arm-cable collisions (align)");
    ...
    LOG_NAMED_INFO("task_factory", "[makeAlign] 已设置ACM允许 arm_group 与缆绳碰撞（对齐），{} 个 link",
  ...
  task.add(makeMoveToJoints("align joint4", align_joint_map, planner));
  
  return task;
}

mtc::Task TaskFactory::makeDescend(const TaskTarget& target) {
  LOG_NAMED_INFO("task_factory", "[makeDescend] 构建下探任务（优先关节目标，否则 OMPL 位姿）");
  
  mtc::Task task;
  if (!ctx_.move_group) {
    LOG_NAMED_ERROR("task_factory", "[makeDescend] move_group 无效，无法构建任务");
    return task;
  }
  
  const auto& robot_model = ctx_.move_group->getRobotModel();
  if (!robot_model) {
    LOG_NAMED_ERROR("task_factory", "[makeDescend] 无法获取 robot model，无法构建任务");
    return task;
  }
  ...
  if (!target.grasp_joint_map.empty()) {
    ...
    planner = cached_joint_planner_;
    LOG_NAMED_INFO("task_factory", "[makeDescend] 使用关节目标 + JointInterpolationPlanner");
  } else {
    planner = createOMPLPlanner();
    if (!planner) {
      LOG_NAMED_ERROR("task_factory", "[makeDescend] PipelinePlanner 创建失败，无法构建任务");
      return task;
    }
    LOG_NAMED_INFO("task_factory", "[makeDescend] grasp_joint_map 为空，使用 OMPL 位姿目标");
  }
  
  // 1. 起始状态：与 pregrasp/align 一致，优先 FixedState（joint_state_diag），避免 planning scene 不同步导致 move to grasp 失败
  addStartStateStageForCartesian(task);
  
  // 2. ACM 允许夹爪与缆绳碰撞
  task.add(makeAllowCollisions());
  LOG_NAMED_INFO("task_factory", "[makeDescend] 已设置ACM允许夹爪与缆绳碰撞");
  
  // 2b. 下探时臂杆会靠近缆绳，需允许 arm_group 所有 link 与缆绳碰撞，否则 JointInterpolationPlanner 会因碰撞判失败
  std::string cable_object_id = ctx_.cable_name + "_world";
  const auto* arm_jmg = robot_model->getJointModelGroup(ctx_.arm_group);
  std::vector<std::string> arm_link_names;
  if (arm_jmg) {
  ...
  if (arm_link_names.empty()) {
    LOG_NAMED_WARN("task_factory", "[makeDescend] arm_group '{}' 未找到或无 link，尝试备用：用非夹爪 link 设置臂-缆绳 ACM",
                   ctx_.arm_group);
    const std::vector<std::string>& all_names = robot_model->getLinkModelNames();
    std::set<std::string> gripper_set(ctx_.allow_touch_links.begin(), ctx_.allow_touch_links.end());
    for (const std::string& link_name : all_names) {
    ...
    if (arm_link_names.empty()) {
      LOG_NAMED_ERROR("task_factory", "[makeDescend] 备用也未找到任何 link，下探可能因碰撞失败");
    } else {
      LOG_NAMED_INFO("task_factory", "[makeDescend] 备用: 为 {} 个非夹爪 link 设置臂-缆绳 ACM", arm_link_names.size());
    }
  }
  if (!arm_link_names.empty()) {
    auto allow_arm = std::make_unique<mtc::stages::ModifyPlanningScene>("allow arm-cable collisions (descend)");
    for (const std::string& link_name : arm_link_names) {
      allow_arm->allowCollisions(cable_object_id, link_name, true);
    }
    task.add(std::move(allow_arm));
    LOG_NAMED_INFO("task_factory", "[makeDescend] 已设置ACM允许 arm_group 与缆绳碰撞（下探），{} 个 link",
                  static_cast<int>(arm_link_names.size()));
  }
  
  // 3. 移到抓取：有关节解则用关节目标，否则用位姿目标
  if (!target.grasp_joint_map.empty()) {
    task.add(makeMoveToJoints("move to grasp", target.grasp_joint_map, planner));
    LOG_NAMED_INFO("task_factory", "[makeDescend] 下探目标: 关节目标 ({} 个关节)", target.grasp_joint_map.size());
  } else {
    task.add(makeMoveToPose("move to grasp", target.grasp_pose, planner));
    LOG_NAMED_INFO("task_factory", "[makeDescend] 下探目标: grasp_pose=({:.3f}, {:.3f}, {:.3f})",
                   target.grasp_pose.pose.position.x, target.grasp_pose.pose.position.y, target.grasp_pose.pose.position.z);
  }
  
  return task;
}

mtc::Task TaskFactory::makeLift(double distance,
                               std::optional<double> joint4_fix_rad,
                               const std::map<std::string, double>* start_joints) {
  if (distance < 0) {
    distance = ctx_.lift_distance;
  }
  LOG_NAMED_INFO("task_factory", "[makeLift] 构建抬升任务, 距离={:.3f}m, joint4_fix={}",
                 distance, joint4_fix_rad ? "是" : "否");
  ...
  // 分支一：以 start_joints（如 grasp_joint_map）为基准计算抬升目标，避免 joint_state_diag 滞后导致无运动
  if (start_joints && !start_joints->empty() && arm_jmg) {
    ...
    if (arm_vals.size() == jnames.size()) {
      ...
      if (state.setFromIK(arm_jmg, target_pose, ctx_.eef_link, 1.0)) {
        ...
        if (use_joint_lift) {
          LOG_NAMED_INFO("task_factory", "[makeLift] 抬升以 grasp_joint_map 为起始，共 {} 个关节，关节解成功",
  ...
  if (!use_joint_lift && ctx_.joint_state_diag && ctx_.moveit_mutex && ctx_.node && arm_jmg) {
    ...
    if (fillJointPositionsFromDiag(ctx_.joint_state_diag,
      ...
      if (state.setFromIK(arm_jmg, target_pose, ctx_.eef_link, 1.0)) {
        ...
        use_joint_lift = !lift_joint_map.empty();
        if (use_joint_lift) {
          LOG_NAMED_INFO("task_factory", "[makeLift] 抬升关节解成功，使用关节目标 + JointInterpolationPlanner");
        }
      }
    }
  }

  // 4-DOF：抬升仅用关节/IK，不用笛卡尔。若 IK 解与起始差异过小（会产生 0 时长轨迹），改用关节空间偏移
  ...
  if (use_joint_lift && !lift_joint_map.empty() && start_joints && !start_joints->empty()) {
    ...
    if (max_diff < lift_joint_eps_rad) {
      LOG_NAMED_INFO("task_factory", "[makeLift] 抬升 IK 与起始差异过小 (max_diff={:.4f} rad)，改用关节空间偏移抬升",
                     max_diff);
  ...
  if (!use_joint_lift && arm_jmg) {
    ...
    if (start_joints && !start_joints->empty()) {
      ...
      for (const auto& name : jnames) {
        auto it = start_joints->find(name);
        if (it == start_joints->end()) break;
        arm_vals.push_back(it->second);
      }
    }
    if (arm_vals.size() != jnames.size() && ctx_.joint_state_diag && ctx_.moveit_mutex && ctx_.node) {
      std::lock_guard<std::mutex> lock(*ctx_.moveit_mutex);
      arm_vals.clear();
      if (fillJointPositionsFromDiag(ctx_.joint_state_diag, jnames, arm_vals,
    ...
    if (arm_vals.size() == jnames.size()) {
      for (size_t i = 0; i < jnames.size(); ++i) {
        double v = arm_vals[i];
        if (jnames[i] == "Joint2") v += ctx_.lift_joint_delta_J2;
        else if (jnames[i] == "Joint3") v += ctx_.lift_joint_delta_J3;
      ...
      LOG_NAMED_INFO("task_factory", "[makeLift] IK 无解，使用关节空间偏移抬升 (J2+{:.3f}, J3+{:.3f} rad，已限位)",
  ...
  if (use_joint_lift && !lift_joint_map.empty()) {
    ...
    task.add(makeMoveToJoints("lift", lift_joint_map, planner));
    LOG_NAMED_INFO("task_factory", "[makeLift] 抬升目标: 关节目标 ({} 个关节)", lift_joint_map.size());
  } else {
    // 4-DOF：不再使用笛卡尔抬升；无关节解且无起始状态时仅记录并返回空任务（FSM 会按失败处理）
    LOG_NAMED_ERROR("task_factory", "[makeLift] 无抬升关节解且无起始关节状态，无法构建抬升任务（已禁用笛卡尔抬升）");
  }

  return task;
}

std::unique_ptr<mtc::stages::ModifyPlanningScene> TaskFactory::makeAddCollisionObject(
  ...
  LOG_NAMED_INFO("task_factory", "[makeAddCollisionObject] 添加碰撞对象: id={}, pos=({:.3f}, {:.3f}, {:.3f})",
...
std::unique_ptr<mtc::stages::ModifyPlanningScene> TaskFactory::makeAllowCollisions() {
  auto stage = std::make_unique<mtc::stages::ModifyPlanningScene>("allow gripper-cable collisions");
  ...
  for (const auto& link : ctx_.allow_touch_links) {
    ...
    LOG_NAMED_DEBUG("task_factory", "[ACM] 允许 {} 与 {} 碰撞", cable_object_id.c_str(), link.c_str());
  }
  
  LOG_NAMED_INFO("task_factory", "[makeAllowCollisions] 设置ACM: 允许{}个夹爪link与缆绳碰撞",
                ctx_.allow_touch_links.size());
  
  return stage;
}

std::unique_ptr<mtc::stages::ModifyPlanningScene> TaskFactory::makeAttachObject() {
  ...
  LOG_NAMED_INFO("task_factory", "[makeAttachObject] 附着碰撞对象 {} 到 {}",
                cable_object_id.c_str(), ctx_.eef_link.c_str());
  
  return stage;
}

std::unique_ptr<mtc::stages::ModifyPlanningScene> TaskFactory::makeDetachObject() {
  ...
  LOG_NAMED_INFO("task_factory", "[makeDetachObject] 分离碰撞对象 {} 从 {}",
                cable_object_id.c_str(), ctx_.eef_link.c_str());
