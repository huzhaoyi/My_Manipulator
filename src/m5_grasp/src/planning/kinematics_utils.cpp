...
void KinematicsUtils::setParameters(
...
geometry_msgs::msg::Quaternion KinematicsUtils::computeDownwardOrientation()
{
  tf2::Quaternion q;
...
geometry_msgs::msg::Quaternion KinematicsUtils::computeDownwardOrientationWithYaw(double yaw)
{
  tf2::Quaternion q;
...
moveit::core::RobotStatePtr KinematicsUtils::getCurrentStateSafe(double timeout)
{
  ...
  try {
    ...
    LOG_NAMED_WARN("ik", "getCurrentStateSafe异常: {}", e.what());
  }
...
std::vector<double> KinematicsUtils::getCurrentJointValuesSafe()
{
  std::lock_guard<std::mutex> lock(moveit_mutex_);
  return move_group_interface_->getCurrentJointValues();
}

geometry_msgs::msg::PoseStamped KinematicsUtils::getCurrentPoseSafe()
{
  std::lock_guard<std::mutex> lock(moveit_mutex_);
...
bool KinematicsUtils::isReachable(double x, double y, double z)
{
  ...
  {
    LOG_NAMED_ERROR("ik",
                "[工作空间] 目标 ({:.3f}, {:.3f}, {:.3f}) 超出最大伸展距离 (d={:.3f} > {:.3f} m)",
  ...
  {
    LOG_NAMED_ERROR("ik",
                "[工作空间] 目标 ({:.3f}, {:.3f}, {:.3f}) 超出最大高度 (z={:.3f} > {:.3f} m)",
                x, y, z, z, max_height);
    return false;
  }
  ...
  {
    LOG_NAMED_WARN("ik",
                "[工作空间] 目标 ({:.3f}, {:.3f}, {:.3f}) 低于最小高度 (z={:.3f} < {:.3f} m)",
                x, y, z, z, min_height);
  }
...
void KinematicsUtils::adjustToWorkspace(double& x, double& y, double& z)
{
  ...
  double distance_from_joint2 = std::sqrt(radius * radius + height_from_joint2 * height_from_joint2);
...
{
  auto state = getCurrentStateSafe(1.0);
  if (!state) {
    return false;
  }
  ...
  pose.position.y = y;
  pose.position.z = z;
  pose.orientation = computeDownwardOrientationWithYaw(yaw);
                                      ...
                                      double tolerance, double step)
{
  ...
  pose.position.y = y;
  pose.position.z = z;
  pose.orientation = computeDownwardOrientationWithYaw(original_yaw);

  if (state->setFromIK(jmg, pose, eef_link_, ik_timeout_)) {
    LOG_NAMED_INFO("ik", 
                "[yaw搜索] 原始yaw={:.3f} rad ({:.1f}°) 可解",
                original_yaw, original_yaw * 180.0 / M_PI);
    return original_yaw;
  }

  LOG_NAMED_WARN("ik", 
              "[yaw搜索] 原始yaw={:.3f} rad ({:.1f}°) 不可解，开始搜索...",
              original_yaw, original_yaw * 180.0 / M_PI);
  ...
  for (int s = 1; s <= max_steps; ++s) {
    ...
    pose.orientation = computeDownwardOrientationWithYaw(yaw_plus);
    ...
    if (test_state->setFromIK(jmg, pose, eef_link_, ik_timeout_)) {
      LOG_NAMED_INFO("ik", 
                  "[yaw搜索] 找到可解yaw: {:.3f} rad ({:.1f}°)，偏移: +{:.3f} rad",
    ...
    pose.orientation = computeDownwardOrientationWithYaw(yaw_minus);
                                              ...
                                              double search_range, double search_step)
{
  ...
  for (double candidate_yaw : candidate_yaws) {
    ...
    pose.position.y = y;
    pose.position.z = z;
    pose.orientation = computeDownwardOrientationWithYaw(candidate_yaw);
...
bool KinematicsUtils::checkJoint23Reachable(const moveit::core::RobotStatePtr& state,
                                             ...
                                             std::vector<double>& joint2_3_values)
{
  ...
  for (size_t i = 0; i < joint_names.size(); ++i) {
    if (joint_names[i] == "Joint2") joint2_idx = i;
    else if (joint_names[i] == "Joint3") joint3_idx = i;
  }
  ...
  if (!joint2_ok || !joint3_ok) {
    LOG_NAMED_WARN("ik", 
                "[Joint2/3检查] 不可达: Joint2={:.3f} rad, Joint3={:.3f} rad",
...
YawIKResult KinematicsUtils::tryYawCandidatesForIK(
    ...
    double ik_timeout)
{
  ...
  auto current_state = getCurrentStateSafe(3.0);
  if (!current_state) {
  ...
  for (size_t i = 0; i < yaw_candidates.size(); ++i) {
    ...
    pose_for_ik.orientation = computeDownwardOrientationWithYaw(candidate_yaw);
    ...
    // 检查Joint2/3可达性
    std::vector<double> joint2_3_values;
    if (checkJoint23Reachable(test_state, "arm_group", joint2_3_values)) {
      LOG_NAMED_INFO("ik", "[YawIK] 候选 #{} (yaw={:.3f}) 可达", 
                  i + 1, candidate_yaw);
...
bool KinematicsUtils::findOptimalYawForJoint4(
    double target_yaw,
    ...
    moveit::core::RobotStatePtr& best_state)
{
  ...
  initial_pose.orientation = computeDownwardOrientationWithYaw(target_yaw);

  moveit::core::RobotStatePtr test_state(new moveit::core::RobotState(*state));
  if (!test_state->setFromIK(jmg, initial_pose, eef_link_, ik_timeout_)) {
    LOG_NAMED_WARN("ik",
                "[Joint4-yaw搜索] 初始yaw={:.3f} rad 在位置({:.3f}, {:.3f}, {:.3f})下IK不可解",
                target_yaw, x, y, z);
    return false;
  }
  ...
  for (int i = 0; i <= search_steps; ++i) {
    ...
    candidate_pose.orientation = computeDownwardOrientationWithYaw(candidate_yaw);
  ...
  if (best_diff < initial_diff - improvement_threshold) {
    LOG_NAMED_INFO("ik",
                "[Joint4-yaw搜索] 找到更优解，差值改善: {:.3f} -> {:.3f} rad",
...
bool KinematicsUtils::checkIKWithCollision(const geometry_msgs::msg::PoseStamped& pose_in_planning)
{
  ...
  moveit::core::GroupStateValidityCallbackFn validity_callback = 
      [&scene](moveit::core::RobotState* st, const moveit::core::JointModelGroup*, const double*) -> bool {
  ...
  bool found = state.setFromIK(
      jmg, pose_in_planning.pose, eef_link_, ik_timeout_, validity_callback);
