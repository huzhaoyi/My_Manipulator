...
void MTCTaskCreator::setBasicParameters(
...
void MTCTaskCreator::setGraspParameters(
...
void MTCTaskCreator::setGripperParameters(
...
void MTCTaskCreator::setTCPParameters(
    double tcp_offset_x,
    double tcp_offset_y,
    double tcp_offset_z)
{
...
void MTCTaskCreator::setJointTolerances(
...
void MTCTaskCreator::setAllowTouchLinks(const std::vector<std::string>& links)
{
  allow_touch_links_ = links;
}

void MTCTaskCreator::setTransformCallback(
    std::function<bool(geometry_msgs::msg::PoseStamped&)> callback)
{
  transform_callback_ = callback;
}

void MTCTaskCreator::setWidthToJointCallback(std::function<double(double)> callback)
{
  width_to_joint_callback_ = callback;
}

void MTCTaskCreator::setOrientationCallbacks(
...
void MTCTaskCreator::setIKCheckCallback(
...
mtc::solvers::PipelinePlannerPtr MTCTaskCreator::createPipelinePlanner(mtc::Task& task)
{
...
double MTCTaskCreator::calculateJoint1Auto(const geometry_msgs::msg::PoseStamped& target_pose)
{
  double joint1_auto = 0.0;
  
  try {
    geometry_msgs::msg::TransformStamped transform_to_base = 
        tf_buffer_->lookupTransform("base_link", target_pose.header.frame_id, tf2::TimePointZero);
    ...
    LOG_NAMED_INFO("mtc", "[Joint1自动计算] 目标在base_link: ({:.3f}, {:.3f}) -> Joint1={:.3f} rad",
...
std::map<std::string, double> MTCTaskCreator::preparePregraspJointMap(
    ...
    bool& use_joint_goal)
{
  ...
  try {
    ...
    YawIKResult ik_result = ik_check_callback_(
        pregrasp_pose.pose, yaw_candidates, eef_link_, 0.5);
...
void MTCTaskCreator::addFixedStartState(mtc::Task& task)
{
  // 与 task_factory::addCurrentStateStage 对齐：使用 CurrentState，由执行端在规划时从 planning scene 取状态。
  // planning scene 由 /joint_states 更新（机械臂 → json → parse_feedback → joint_state_broadcaster → /joint_states），
  // 不再在本节点调用 getCurrentJointValues/getCurrentState，避免时间戳/拉取失败导致流程卡住。
...
void MTCTaskCreator::addOpenGripperStage(mtc::Task& task, mtc::solvers::PipelinePlannerPtr planner)
{
...
void MTCTaskCreator::addCloseGripperStage(mtc::Task& task, mtc::solvers::PipelinePlannerPtr planner)
{
...
void MTCTaskCreator::addMoveToPregraspStage(
    mtc::Task& task,
...
void MTCTaskCreator::addCableCollisionObject(
    mtc::Task& task,
    ...
    double cable_yaw)
{
  ...
  if (make_cable_cylinder_pose_callback_) {
    geometry_msgs::msg::Pose cylinder_pose = make_cable_cylinder_pose_callback_(cable_pose, cable_yaw);
...
void MTCTaskCreator::addAllowCollisionsStage(mtc::Task& task)
{
...
void MTCTaskCreator::addDescendStage(
    mtc::Task& task,
    mtc::solvers::PipelinePlannerPtr planner,
    const geometry_msgs::msg::PoseStamped& grasp_pose)
{
  ...
  auto move_to_grasp = std::make_unique<mtc::stages::MoveTo>("descend to grasp", cartesian_planner);
...
void MTCTaskCreator::addAttachObjectStage(mtc::Task& task)
{
...
void MTCTaskCreator::addLiftStage(
    mtc::Task& task,
    mtc::solvers::PipelinePlannerPtr planner)
{
  auto cartesian_planner = std::make_shared<mtc::solvers::CartesianPath>();
...
mtc::Task MTCTaskCreator::createGraspTask(
    const geometry_msgs::msg::PoseStamped& cable_pose,
    double cable_yaw,
    double joint4_target)
{
  ...
  auto pipeline_planner = createPipelinePlanner(task);
  ...
  // 计算Joint1
  double joint1_auto = calculateJoint1Auto(cable_pose_planning);
  ...
  std::map<std::string, double> pregrasp_joint_map = preparePregraspJointMap(
  ...
  // 添加各个阶段
  addFixedStartState(task);
  addOpenGripperStage(task, pipeline_planner);
  addMoveToPregraspStage(task, pipeline_planner, pregrasp_pose, pregrasp_joint_map, 
                          use_joint_goal, joint1_auto, joint4_target);
  
  if (add_collision_object_) {
    addCableCollisionObject(task, cable_pose_planning, selected_yaw);
    addAllowCollisionsStage(task);
  }
  
  addDescendStage(task, pipeline_planner, grasp_pose);
  addCloseGripperStage(task, pipeline_planner);
  addAttachObjectStage(task);
  addLiftStage(task, pipeline_planner);
...
mtc::Task MTCTaskCreator::createPlaceTask(
...
mtc::Task MTCTaskCreator::createGraspAndPlaceTask(
    const geometry_msgs::msg::PoseStamped& cable_pose,
    const geometry_msgs::msg::PoseStamped& place_pose,
    double cable_yaw,
    double joint4_target)
{
  ...
  // 可以在后续版本中扩展
  return createGraspTask(cable_pose, cable_yaw, joint4_target);
}
