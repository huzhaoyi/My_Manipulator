...
SceneManager::SceneManager(
    ...
    : node_(node), planning_scene_interface_(planning_scene_interface),
      move_group_interface_(move_group_interface), tf_buffer_(tf_buffer),
      moveit_mutex_(moveit_mutex)
{
}

void SceneManager::set_parameters(const std::string& planning_frame, const std::string& eef_link,
                                 double cable_diameter, double cable_length,
                                 double cable_center_offset_z, double ground_height,
...
bool SceneManager::transform_pose_to_planning(geometry_msgs::msg::PoseStamped& pose)
{
    ...
    if (pose.header.stamp.sec != 0 || pose.header.stamp.nanosec != 0)
    {
        ...
        if (tf_buffer_->canTransform(planning_frame_, pose.header.frame_id, transform_time,
                                     timeout))
        {
            try
            {
                geometry_msgs::msg::TransformStamped transform = tf_buffer_->lookupTransform(
                    planning_frame_, pose.header.frame_id, transform_time);
            ...
            {
                LOG_NAMED_DEBUG("scene", "canTransform通过但lookup失败，fallback到最新TF: {}",
                                ex.what());
            }
        }
    }
    ...
    {
        LOG_THROTTLE_WARN(5000, "TF转换fallback次数过多（{}次）", tf_fallback_count_.load());
    }
    ...
    try
    {
        ...
        if (tf_buffer_->canTransform(planning_frame_, pose.header.frame_id, tf2::TimePointZero,
                                     timeout))
        {
            geometry_msgs::msg::TransformStamped transform = tf_buffer_->lookupTransform(
                planning_frame_, pose.header.frame_id, tf2::TimePointZero);
            tf2::doTransform(pose, pose, transform);
            pose.header.frame_id = planning_frame_;
            return true;
        }
...
SceneManager::make_cable_cylinder_pose(const geometry_msgs::msg::PoseStamped& cable_pose_planning,
                                    double cable_yaw)
{
    ...
    LOG_NAMED_INFO(
        ...
        cable_yaw, cable_yaw * 180.0 / M_PI, pose.orientation.x, pose.orientation.y,
        pose.orientation.z, pose.orientation.w);

    return pose;
}

bool SceneManager::add_cable_object(const geometry_msgs::msg::PoseStamped& cable_pose,
                                  double cable_yaw, const std::string& object_id)
{
    ...
    bool object_exists =
        std::find(known_objects.begin(), known_objects.end(), object_id) != known_objects.end();

    // 创建碰撞对象
    moveit_msgs::msg::CollisionObject collision_object;
    collision_object.header.frame_id = planning_frame_;
    collision_object.id = object_id;
    collision_object.operation = object_exists ? moveit_msgs::msg::CollisionObject::MOVE
...
bool SceneManager::add_ground_plane()
{
    std::lock_guard<std::mutex> lock(moveit_mutex_);
    ...
    std::vector<std::string> known_objects = planning_scene_interface_->getKnownObjectNames();
    bool ground_exists =
        std::find(known_objects.begin(), known_objects.end(), ground_id) != known_objects.end();

    moveit_msgs::msg::CollisionObject collision_object;
    collision_object.header.frame_id = planning_frame_;
    collision_object.id = ground_id;
    collision_object.operation = ground_exists ? moveit_msgs::msg::CollisionObject::MOVE
                                               : moveit_msgs::msg::CollisionObject::ADD;
    ...
    double final_ground_height =
        (ground_height_ != 0.0) ? ground_height_ : calculated_ground_height;
...
bool SceneManager::is_object_attached(const std::string& object_id, const std::string& eef_link)
{
    try
    {
...
bool SceneManager::attach_object(const std::string& object_id, const std::string& eef_link,
                                const geometry_msgs::msg::PoseStamped& cable_pose, double cable_yaw)
{
    std::lock_guard<std::mutex> lock(moveit_mutex_);
    ...
    geometry_msgs::msg::Pose cylinder_pose_planning =
        make_cable_cylinder_pose(pose_in_planning, cable_yaw);
    ...
    // 先移除旧附着
    if (is_object_attached(object_id, eef_link))
    {
...
bool SceneManager::remove_object(const std::string& object_id)
{
    std::lock_guard<std::mutex> lock(moveit_mutex_);
...
bool SceneManager::detach_and_remove(const std::string& object_id, const std::string& eef_link)
{
    std::lock_guard<std::mutex> lock(moveit_mutex_);

    std::vector<std::string> known_objects = planning_scene_interface_->getKnownObjectNames();
    bool world_object_exists =
        std::find(known_objects.begin(), known_objects.end(), object_id) != known_objects.end();
