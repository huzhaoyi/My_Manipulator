...
def generate_launch_description():
    ...
    with suppress_moveit_warnings():
        moveit_config = (
            ...
            .joint_limits(file_path=os.path.join(sealien_payload_moveit_config_dir, "config", "joint_limits.yaml"))
            .sensors_3d(file_path=os.path.join(sealien_payload_moveit_config_dir, "config", "sensors_3d.yaml"))  # 添加传感器配置（禁用Octomap）
    ...
    move_group_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(sealien_payload_bringup_dir, "launch", "move_group.launch.py")
        )
    )
