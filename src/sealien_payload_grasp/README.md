# sealien_payload_grasp（Sealien CtrlPilot Payload 抓取节点）

机械臂抓取节点 - 基于 MoveIt Task Constructor 的模块化实现。

## 架构

```
sealien_payload_grasp_node (ROS2 Node, 454行)
...
| 模块 | 路径 | 职责 |
|------|------|------|
| GraspNode | `src/sealien_payload_grasp.cpp` | ROS2资源管理、模块协调 |
...
| 话题 | 类型 | 方向 | 说明 |
|------|------|------|------|
| `/cable_pose_with_yaw` | `sealien_payload_msgs/CablePoseWithYaw` | 订阅 | 缆绳位置+yaw |
...
```bash
# 启动节点
ros2 run sealien_payload_grasp sealien_payload_grasp_node
...
| `sealien_payload_msgs` | 自定义消息（CablePoseWithYaw） |
...
sealien_payload_grasp_node
    │
    ├── sealien_payload_grasp_logging_lib        # 日志库（spdlog封装）
    │       └── spdlog
    │
    ├── sealien_payload_grasp_utils_lib          # 工具库
    │       ├── trajectory_planner  # 轨迹规划
    │       ├── scene_manager       # 场景管理
    │       └── parameter_manager   # 参数管理
    │
    ├── sealien_payload_grasp_fsm_lib            # FSM库
    │       ├── target_tracker      # 目标跟踪
    │       ├── grasp_fsm           # 抓取状态机
    │       └── sealien_payload_grasp_mtc_lib    # 依赖MTC库
    │
    ├── sealien_payload_grasp_mtc_lib            # MTC库
    │       ├── task_context        # 任务上下文
    │       ├── task_factory        # 任务构建
    │       └── task_runner         # 任务执行
    │
    ├── sealien_payload_grasp_planning_lib       # 规划库
    │       ├── kinematics_utils    # IK工具
    │       └── mtc_task_creator    # MTC任务创建
    │
    ├── sealien_payload_grasp_execution_lib      # 执行库
    │       ├── gripper_controller  # 夹爪控制（直接 UDP，读/写 tmp 文件）
    │       ├── motion_executor     # 运动执行
    │       └── cartesian_executor  # 笛卡尔执行
    │
    └── sealien_payload_grasp_visualization_lib  # 可视化库
