...
struct LogConfig
{
    ...
    std::string log_dir = "/tmp/sealien_payload_grasp_logs";
    std::string file_prefix = "sealien_payload_grasp";
...
#define M5_LOG_IMPL(spdlog_level, module, ...)                                                     \
    do                                                                                             \
    {                                                                                              \
        auto& mgr = sealien_payload_grasp::logging::LogManager::instance();                                     \
...
#define LOG_THROTTLE_IMPL(spdlog_level, module, interval_ms, ...)                                  \
    do                                                                                             \
    {                                                                                              \
        if (sealien_payload_grasp::logging::ThrottleController::instance().shouldLog(__FILE__, __LINE__,        \
                                                                        interval_ms))              \
        {                                                                                          \
...
#define LOG_ONCE_IMPL(spdlog_level, module, ...)                                                   \
    do                                                                                             \
    {                                                                                              \
        if (sealien_payload_grasp::logging::ThrottleController::instance().shouldLogOnce(__FILE__, __LINE__))   \
        {                                                                                          \
            M5_LOG_IMPL(spdlog_level, module, __VA_ARGS__);                                        \
        }                                                                                          \
    } while (0)
