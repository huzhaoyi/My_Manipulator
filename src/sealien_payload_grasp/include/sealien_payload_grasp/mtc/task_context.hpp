 ...
 * 数据链：机械臂 UDP JSON → M5HardwareInterface::parse_feedback_json → joint_state_broadcaster →
 * /joint_states。 MTC 首 stage 已按官方教程用 CurrentState（执行端从 planning scene
 * 取）；本结构用于诊断及 其他需要「最新 /joint_states」的路径（如 IK 种子、get_current_state_safe
 * 后备等）。
 */
...
struct TaskContext
{
    ...
    std::shared_ptr<rclcpp::executors::MultiThreadedExecutor> executor{
        nullptr}; // Executor引用，用于spin_until_future_complete
    rclcpp::CallbackGroup::SharedPtr callback_group_fast{
        nullptr}; // 快速callback group，用于action client回调

    /** 轻量 joint_states 诊断：last_js_wall/stamp、js_count、1Hz 节流，用于定位「没收到」vs「stamp
     * 老」 */
    mutable std::shared_ptr<JointStateDiag> joint_state_diag{nullptr};

    double min_joint_step_rad{0.175};

    double range_execution_time_joint1{30.0};
    double range_execution_time_joint2{13.0};
    double range_execution_time_joint3{13.0};
    double range_execution_time_joint4{21.0};
    double range_execution_time_gripper{7.5};

    double execution_timeout_margin_s{5.0};
    double min_execution_timeout_s{35.0};
    double max_execution_timeout_s{300.0};
    double execution_timeout_safety_factor{1.5};
    double min_segment_time_s{0.05};
    double min_arm_trajectory_time_s{1.0};
     ...
     */
    moveit::core::RobotStatePtr get_current_state_safe() const;
};
 ...
 * 供 get_current_state_safe、cleanupMoveGroupState、compute_from_message
 * 等需要「实际机械臂数值」后备时使用。 仅当缓存“新鲜”时才返回 true：now−last_js_stamp 或
 * now−last_js_wall 必须小于 max_age_sec。
...
{
    // 原始目标
    sealien_payload_msgs::msg::CablePoseWithYaw cable_pose_msg;
     ...
     * @return 是否计算成功
     */
    bool compute_from_message(const TaskContext& ctx, const sealien_payload_msgs::msg::CablePoseWithYaw& msg);
};
