...
#include "sealien_payload_grasp/mtc/task_factory.hpp"
...
struct ExecWaitState
{
    ...
    rclcpp_action::ClientGoalHandle<
        moveit_task_constructor_msgs::action::ExecuteTaskSolution>::SharedPtr goal_handle;
};
...
class ITaskRunner
{
  public:
     ...
     * @return true 如果执行成功
     */
    virtual bool run_pregrasp(const TaskTarget& target) = 0;
     ...
     * @return true 如果执行成功
     */
    virtual bool run_align(const TaskTarget& target) = 0;
     ...
     * @return true 如果执行成功
     */
    virtual bool run_descend(const TaskTarget& target) = 0;
     ...
     * @return true 如果执行成功
     */
    virtual bool run_lift(const TaskTarget& target) = 0;

    /**
     * @brief 设置急停标志
     */
    virtual void set_emergency_stop(bool stop) = 0;

    /**
     * @brief 检查是否处于急停状态
     */
    virtual bool is_emergency_stopped() const = 0;
};
...
class TaskRunner : public ITaskRunner, public std::enable_shared_from_this<TaskRunner>
{
  public:
    ...
    // ITaskRunner 接口实现
    bool run_pregrasp(const TaskTarget& target) override;
    bool run_align(const TaskTarget& target) override;
    bool run_descend(const TaskTarget& target) override;
    bool run_lift(const TaskTarget& target) override;
    void set_emergency_stop(bool stop) override;
    bool is_emergency_stopped() const override;
     ...
     * @return true 如果执行成功
     */
    bool execute_task(mtc::Task& task, const std::string& task_name);

    /**
     * @brief 初始化 action client（延迟初始化，确保 ctx_ 已准备好）
     * @return true 如果初始化成功
     */
    bool initialize_action_client();

    /**
     * @brief 检查当前执行状态（供 FSM 调用）
     * @return 0=未开始, 1=执行中, 2=成功, 3=失败, 4=超时
     */
    int check_execution_status() const;
     ...
     */
    std::string get_current_task_name() const;

    /**
     * @brief 启动非阻塞机械臂稳定检查（每次 tick 调用 tick_arm_stable_check 一次）
     ...
     */
    void start_arm_stable_check(int timeout_ms, double threshold_rad, int stable_window,
                             double velocity_eps_rad_s = 0.0, int min_dwell_ms = 0);

    /**
     * @brief 非阻塞轮询一次稳定检查（不 sleep，供 FSM 每 tick 调用）
     * @param now 当前时间（与 start_arm_stable_check 同源时钟）
     * @return (done, stable)：done 为 true 表示可进入下一步，stable 表示是否达到稳定（false
     * 表示超时）
     */
    std::pair<bool, bool> tick_arm_stable_check(const rclcpp::Time& now);

    /**
     * @brief 设置“下发前”回调：在修复并设置 controller 后的 solution 发送给 ExecuteTaskSolution
     * 前调用，用于将轨迹发布到网页等
     */
    void set_on_solution_before_send(
        std::function<void(const moveit_task_constructor_msgs::msg::Solution&)> cb)
    {
  ...
  private:
     ...
     */
    bool execute_planned_task(mtc::Task& task, const std::string& task_name);

    /**
     * @brief 清理MoveGroup状态（避免残留约束）
     */
    void cleanup_move_group_state();
     ...
     */
    void fix_trajectory_timestamps(moveit_task_constructor_msgs::msg::Solution& sol_msg);
     ...
     */
    void set_controller_names_for_solution(moveit_task_constructor_msgs::msg::Solution& sol_msg);

    std::function<void(const moveit_task_constructor_msgs::msg::Solution&)>
        on_solution_before_send_;
    ...
    // 解决：execute_planned_task() 异步化，FSM 通过定期检查状态推进
