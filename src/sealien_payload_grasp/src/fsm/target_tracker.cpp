#include "sealien_payload_grasp/fsm/target_tracker.hpp"
...
void TargetTracker::push(const sealien_payload_msgs::msg::CablePoseWithYaw& msg, const rclcpp::Time& stamp)
{
    std::lock_guard<std::mutex> lock(mutex_);
...
std::optional<sealien_payload_msgs::msg::CablePoseWithYaw> TargetTracker::getStableTarget() const
{
    std::lock_guard<std::mutex> lock(mutex_);
...
std::optional<sealien_payload_msgs::msg::CablePoseWithYaw> TargetTracker::getLatestTarget() const
{
    std::lock_guard<std::mutex> lock(mutex_);
    if (has_target_)
    {
...
bool TargetTracker::isJumping(const sealien_payload_msgs::msg::CablePoseWithYaw& ref) const
{
    std::lock_guard<std::mutex> lock(mutex_);
    if (!has_target_)
    {
...
bool TargetTracker::is_valid(const rclcpp::Time& now,
                            const sealien_payload_msgs::msg::CablePoseWithYaw& ref) const
{
    if (isStale(now))
    {
