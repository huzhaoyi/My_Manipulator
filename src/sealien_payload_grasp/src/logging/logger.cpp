...
void crashSignalHandler(int sig)
{
    ...
    std::string crash_file =
        "/tmp/sealien_payload_grasp_crash_" +
...
void LogManager::setupCrashHandler()
{
    ...
    std::set_terminate(
        ...
        {
            LogManager::instance().flushAll();
            LogManager::instance().dumpRingBuffer("/tmp/sealien_payload_grasp_terminate.log");
            std::abort();
        });
}
...
void LogManager::dumpRingBuffer(const std::string& filename)
{
    ...
    try
    {
        ...
        if (out.is_open())
        {
            out << "=== Sealien CtrlPilot Payload (m5_grasp) Crash Dump ===\n";
