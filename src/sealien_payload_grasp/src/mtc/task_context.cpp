...
void log_state_freshness_if_needed(const TaskContext& ctx)
{
    if (!ctx.joint_state_diag || !ctx.node)
        return;
    auto now = ctx.node->get_clock()->now();
    auto& d = *ctx.joint_state_diag;
    std::lock_guard<std::mutex> lock(d.m);
    if (d.last_log_time.nanoseconds() != 0 && (now - d.last_log_time).seconds() < 1.0)
        return;
    ...
    LOG_NAMED_INFO("state_diag", "now−last_js_wall={:.2f}s now−last_js_stamp={:.2f}s js_count={}",
                   dt_wall_sec, dt_stamp_sec, d.js_count);
    d.last_log_time = now;
}

bool fill_joint_positions_from_diag(const std::shared_ptr<JointStateDiag>& diag,
                                const std::vector<std::string>& joint_names,
                                std::vector<double>& out, const rclcpp::Time& now,
                                double max_age_sec)
{
    out.clear();
    if (!diag || joint_names.empty())
        return false;
    ...
    // 2) 新鲜度检查：仅用与 now 同源的 last_js_wall，避免 "can't subtract times with different time
    // sources" （last_js_stamp 来自 msg->header，可能与 node 的 clock 类型不同，如 ROS vs Steady）
...
bool fill_joint_velocities_from_diag(const std::shared_ptr<JointStateDiag>& diag,
                                 const std::vector<std::string>& joint_names,
                                 std::vector<double>& out, const rclcpp::Time& now,
                                 double max_age_sec)
{
 ...
 */
bool check_joint23_reachable(const moveit::core::RobotState& state,
                           const moveit::core::JointModelGroup* jmg, double& joint2_value,
                           double& joint3_value)
{
    if (!jmg)
        return false;
    ...
    for (size_t i = 0; i < joint_names.size(); ++i)
    {
        if (joint_names[i] == "Joint2")
            joint2_idx = static_cast<int>(i);
        else if (joint_names[i] == "Joint3")
            joint3_idx = static_cast<int>(i);
    }
 ...
 * @return true 如果找到可达解
 */
bool try_yaw_candidates_for_ik(const geometry_msgs::msg::Pose& pose,
                           const std::vector<double>& yaw_candidates,
                           moveit::core::RobotState& robot_state,
                           const moveit::core::JointModelGroup* jmg, const std::string& eef_link,
                           std::vector<double>& out_joint_values, double& out_selected_yaw)
{
    if (!jmg || yaw_candidates.empty())
        return false;
    ...
    for (size_t i = 0; i < yaw_candidates.size(); ++i)
    {
        ...
        LOG_NAMED_INFO("task_target", "[Yaw候选IK] 尝试候选#{}: yaw={:.1f}°", i + 1,
        ...
        if (check_joint23_reachable(robot_state, jmg, joint2_val, joint3_val))
        {
            LOG_NAMED_INFO("task_target",
                           "[Yaw候选IK] ✓ 候选#{} 可达！Joint2={:.1f}°, Joint3={:.1f}°", i + 1,
        ...
        else
        {
            LOG_NAMED_WARN(
                "task_target",
                "[Yaw候选IK] 候选#{} IK可解但Joint2/3不可达: Joint2={:.1f}°, Joint3={:.1f}°", i + 1,
                joint2_val * 180.0 / M_PI, joint3_val * 180.0 / M_PI);
        }
    }
...
bool TaskTarget::compute_from_message(const TaskContext& ctx,
                                    const sealien_payload_msgs::msg::CablePoseWithYaw& msg)
{
    valid = false;
    cable_pose_msg = msg;
    ...
    joint4_target = normalize_angle(joint4_target);

    LOG_NAMED_INFO("task_target", "Joint4目标计算: yaw={:.1f}° + offset={:.1f}° {} = {:.1f}°",
                   msg.yaw * 180.0 / M_PI, ctx.yaw_offset * 180.0 / M_PI,
                   ctx.yaw_flip ? "+ 180°" : "", joint4_target * 180.0 / M_PI);
    ...
    if (ctx.move_group && ctx.moveit_mutex)
    {
        try
        {
            ...
            if (robot_model)
            {
                ...
                if (arm_jmg)
                {
                    ...
                    {
                        seed_ok = fill_joint_positions_from_diag(ctx.joint_state_diag, arm_names,
                                                             current_joints, now, 0.5);
                        if (seed_ok)
                        {
                            LOG_NAMED_INFO("task_target",
                                           "[预抓取IK] 使用 joint_state_diag 作为关节种子");
                        }
                    }
                    if (!seed_ok)
                    {
                        ...
                        {
                            if (fill_joint_positions_from_diag(ctx.joint_state_diag, arm_names,
                                                           current_joints, now, 0.5))
                            {
                                LOG_NAMED_INFO("task_target",
                                               "[预抓取IK] getCurrentJointValues 不可用，回退 "
                                               "joint_state_diag 作为关节种子");
                            }
                        }
                    }
                    ...
                    if (current_joints.size() >= 4)
                    {
                        ...
                        {
                            LOG_NAMED_WARN("task_target", "检测到全零状态（奇异点附近）；若IK解出将"
                                                          "使用关节目标以避免OMPL位姿采样失败");
                        }
                    }
                    ...
                    LOG_NAMED_INFO("task_target",
                                   "[预抓取IK] 尝试yaw候选: 原始yaw={:.1f}°, 翻转yaw={:.1f}°",
                    ...
                    bool ik_found =
                        try_yaw_candidates_for_ik(pregrasp_pose.pose, yaw_candidates, *robot_state,
                                              arm_jmg, ctx.eef_link, ik_joint_values, selected_yaw);

                    if (ik_found)
                    {
                        // IK 成功则始终使用关节目标，避免 OMPL 对位姿目标采不到有效 goal（Unable to
                        // sample any valid states for goal tree）
                        const auto& joint_names = arm_jmg->getActiveJointModelNames();
                        pregrasp_joint_map.clear();
                        for (size_t i = 0; i < joint_names.size() && i < ik_joint_values.size();
                             ++i)
                        {
                        ...
                        // 检查Joint2/3是否在合理范围
                        double joint2_val =
                            pregrasp_joint_map.count("Joint2") ? pregrasp_joint_map["Joint2"] : 0.0;
                        double joint3_val =
                            pregrasp_joint_map.count("Joint3") ? pregrasp_joint_map["Joint3"] : 0.0;
                        LOG_NAMED_INFO("task_target",
                                       "最终关节目标: Joint2={:.1f}°, Joint3={:.1f}°",
                                       joint2_val * 180.0 / M_PI, joint3_val * 180.0 / M_PI);
                        ...
                        if (is_near_zero_state)
                        {
                            LOG_NAMED_INFO(
                                "task_target",
                                "全零状态，但因IK已解出仍使用关节目标模式（避免OMPL位姿采样失败）");
                        }
                        else
                        {
                            LOG_NAMED_INFO("task_target",
                                           "IK计算成功（使用yaw={:.1f}°），使用关节目标模式",
                                           selected_yaw * 180.0 / M_PI);
                        }
                        for (const auto& [name, value] : pregrasp_joint_map)
                        {
                            LOG_NAMED_INFO("task_target", "  {}: {:.1f}°", name,
                                           value * 180.0 / M_PI);
                        }
                    }
                    else
                    {
                        ...
                        {
                            LOG_NAMED_WARN("task_target", "所有yaw候选IK都失败，将使用位姿目标模式"
                                                          "（可能因为目标超出工作空间）");
                        }
                        use_joint_goal = false;
                    }

                    // 8. 对 grasp_pose 做 IK，供下探使用关节目标（避免 OMPL 对 pose 采不到有效
                    // goal）
                    ...
                    bool grasp_ik_ok = try_yaw_candidates_for_ik(grasp_pose.pose, yaw_candidates,
                                                             *robot_state, arm_jmg, ctx.eef_link,
                                                             grasp_ik_values, grasp_sel_yaw);
                    if (grasp_ik_ok)
                    {
                        const auto& jnames = arm_jmg->getActiveJointModelNames();
                        ...
                        if (grasp_joint_map.count("Joint1"))
                            grasp_joint_map["Joint1"] = joint1_auto;
                        if (grasp_joint_map.count("Joint4"))
                            grasp_joint_map["Joint4"] = joint4_target;
                        LOG_NAMED_INFO("task_target",
                                       "[下探IK] grasp_pose 关节解成功，将用关节目标下探");
                    }
                }
            }
        ...
        catch (const std::exception& e)
        {
            ...
            LOG_NAMED_ERROR("task_target",
                            "compute_from_message 预抓取IK阶段异常: "
                            "{}，将退回位姿目标模式（use_joint_goal=false）",
                            e.what());
            use_joint_goal = false;
            pregrasp_joint_map.clear();
            grasp_joint_map.clear();
        ...
        {
            LOG_NAMED_ERROR("task_target",
                            "compute_from_message "
                            "预抓取IK阶段发生未知异常，将退回位姿目标模式（use_joint_goal=false）");
            use_joint_goal = false;
            pregrasp_joint_map.clear();
            grasp_joint_map.clear();
            try
...
moveit::core::RobotStatePtr TaskContext::get_current_state_safe() const
{
    log_state_freshness_if_needed(*this);
    if (!move_group || !moveit_mutex)
    {
        LOG_NAMED_WARN("task_context", "get_current_state_safe: 上下文无效");
        return nullptr;
    }
    ...
    // 原因：MoveGroupInterface::getCurrentJointValues()/getCurrentState() 内部会等待
    // CurrentStateMonitor 的“recent”
    // joint_states，若短时间内没收到（DDS发现/回调阻塞/偶发掉帧）会报
    // "Didn't receive robot state... latest received state has time 0"。
    // 而本项目已有同进程的 best_effort/volatile /joint_states
    // 轻量订阅（joint_state_diag），更直接可靠。
    try
    {
        const auto& robot_model = move_group->getRobotModel();
        if (!robot_model)
        {
            LOG_NAMED_WARN("task_context", "get_current_state_safe: 无法获取机器人模型");
            return nullptr;
        }
        ...
        if (arm_jmg)
        {
            ...
            if (joint_state_diag)
            {
                ok_from_diag = fill_joint_positions_from_diag(joint_state_diag, arm_names,
                                                          arm_joint_values, now, 0.5);
                if (ok_from_diag)
                {
                    LOG_NAMED_INFO("task_context",
                                   "get_current_state_safe: arm 使用 joint_state_diag");
                }
            }
            if (!ok_from_diag)
            {
            ...
            {
                LOG_NAMED_WARN("task_context",
                               "get_current_state_safe: arm 关节数量不匹配 (got={}, expected={})",
        ...
        if (gripper_jmg && gripper_group_interface)
        {
            ...
            if (joint_state_diag)
            {
                ok_from_diag = fill_joint_positions_from_diag(joint_state_diag, gripper_names,
                                                          gripper_joint_values, now, 0.5);
                if (ok_from_diag)
                {
                    LOG_NAMED_INFO("task_context",
                                   "get_current_state_safe: gripper 使用 joint_state_diag");
                }
            }
            if (!ok_from_diag)
            {
    ...
    {
        LOG_NAMED_WARN("task_context", "getCurrentJointValues()失败: {}，尝试使用getCurrentState()",
                       e.what());
    }
    ...
    if (joint_state_diag)
    {
        ...
        if (robot_model)
        {
            ...
            if (arm_jmg)
            {
                ...
                if (fill_joint_positions_from_diag(
                        joint_state_diag, arm_jmg->getActiveJointModelNames(), arm_vals, now, 0.5))
                {
            ...
            if (gripper_jmg && gripper_group_interface)
            {
                ...
                if (fill_joint_positions_from_diag(joint_state_diag,
                                               gripper_jmg->getActiveJointModelNames(), grip_vals,
                                               now, 0.5))
                {
            ...
            if (arm_filled)
            {
                state->update();
                state->enforceBounds();
                LOG_NAMED_INFO("task_context", "get_current_state_safe: 使用 joint_state_diag 后备");
                return state;
            }
        }
    }
