...
trajectory_msgs::msg::JointTrajectory TrajectoryPlanner::generate_cubic_polynomial_trajectory(
    ...
    const std::vector<std::string>& joint_names, double dt)
{
    // 参数验证
    ...
    std::vector<double> start_vel_actual =
        start_vel.empty() ? std::vector<double>(num_joints, 0.0) : start_vel;
    std::vector<double> end_vel_actual =
        end_vel.empty() ? std::vector<double>(num_joints, 0.0) : end_vel;
    ...
    if (!trajectory.points.empty())
    {
        ...
        last_point.time_from_start.nanosec =
            static_cast<uint32_t>((duration - static_cast<int>(duration)) * 1e9);
    }

    return trajectory;
}

trajectory_msgs::msg::JointTrajectory TrajectoryPlanner::generate_quintic_polynomial_trajectory(
    const std::vector<double>& start_joints, const std::vector<double>& end_joints,
    ...
    const std::vector<std::string>& joint_names, double dt)
{
    ...
    for (size_t joint_idx = 0; joint_idx < num_joints; ++joint_idx)
    {
        ...
        double a3_coeff =
            (20.0 * (qf - q0) - (8.0 * vf + 12.0 * v0) * T - (3.0 * a0 - af) * T2) / (2.0 * T3);
        double a4_coeff =
            (30.0 * (q0 - qf) + (14.0 * vf + 16.0 * v0) * T + (3.0 * a0 - 2.0 * af) * T2) / T4;
        ...
        for (int i = 0; i < num_points; ++i)
        {
            ...
            // 计算位置
            double position = a0_coeff + a1_coeff * t + a2_coeff * t2 + a3_coeff * t3 +
                              a4_coeff * t4 + a5_coeff * t5;

            // 计算速度
            double velocity = a1_coeff + 2.0 * a2_coeff * t + 3.0 * a3_coeff * t2 +
                              4.0 * a4_coeff * t3 + 5.0 * a5_coeff * t4;

            // 计算加速度
            double acceleration =
                2.0 * a2_coeff + 6.0 * a3_coeff * t + 12.0 * a4_coeff * t2 + 20.0 * a5_coeff * t3;
...
TrajectoryPlanner::generate_linear_cartesian_trajectory(const geometry_msgs::msg::Pose& start_pose,
                                                     ...
                                                     double duration, double dt)
{
    ...
    // 姿态SLERP插补
    geometry_msgs::msg::Quaternion q0 = normalize_quaternion(start_pose.orientation);
    geometry_msgs::msg::Quaternion qf = normalize_quaternion(end_pose.orientation);
...
trajectory_msgs::msg::JointTrajectory TrajectoryPlanner::generate_bspline_joint_trajectory(
    const std::vector<std::vector<double>>& control_points, int degree, double duration,
    const std::vector<std::string>& joint_names, double dt)
{
    // 参数验证
    ...
    // 生成节点向量
    std::vector<double> knots = generate_knot_vector(control_points.size(), degree);
    ...
    for (int i = 0; i < num_points; ++i)
    {
        ...
        for (size_t joint_idx = 0; joint_idx < num_joints; ++joint_idx)
        {
            ...
            {
                double basis = bspline_basis(i, degree, u_mapped, knots);
            ...
            for (size_t i = 0; i < control_points.size(); ++i)
            {
                pos_plus += control_points[i][joint_idx] * bspline_basis(i, degree, u_plus, knots);
                pos_minus += control_points[i][joint_idx] * bspline_basis(i, degree, u_minus, knots);
            }
            ...
            {
                pos_plus2 +=
                    control_points[i][joint_idx] * bspline_basis(i, degree, u_plus + h, knots);
                pos_minus2 +=
                    control_points[i][joint_idx] * bspline_basis(i, degree, u_minus - h, knots);
            }
...
std::vector<geometry_msgs::msg::Pose> TrajectoryPlanner::generate_bspline_cartesian_trajectory(
    const std::vector<geometry_msgs::msg::Pose>& control_points, int degree, double duration,
    double dt)
{
    // 参数验证
    if (control_points.empty())
    ...
    for (int i = 0; i < num_points; ++i)
    {
        ...
        {
            double basis = bspline_basis(j, degree, u_mapped, knots);
        ...
        geometry_msgs::msg::Quaternion q0 =
            normalize_quaternion(control_points[segment_idx].orientation);
        geometry_msgs::msg::Quaternion q1 =
            normalize_quaternion(control_points[segment_idx + 1].orientation);
...
double TrajectoryPlanner::bspline_basis(int i, int k, double t, const std::vector<double>& knots)
{
    // Cox-de Boor递归算法
    if (k == 0)
    {
...
std::vector<double> TrajectoryPlanner::generate_knot_vector(int num_control_points, int degree)
{
...
geometry_msgs::msg::Quaternion TrajectoryPlanner::slerp(const geometry_msgs::msg::Quaternion& q1,
...
TrajectoryPlanner::normalize_quaternion(const geometry_msgs::msg::Quaternion& q)
{
...
bool TrajectoryPlanner::apply_time_parameterization(
    trajectory_msgs::msg::JointTrajectory& trajectory,
    const moveit::core::RobotModelConstPtr& robot_model, const std::string& group_name,
    double max_velocity_scaling, double max_acceleration_scaling,
    TimeParameterizationType algorithm)
{
    ...
    // 应用时间参数化
    bool success = apply_time_parameterization(robot_traj, max_velocity_scaling,
    ...
    for (size_t i = 0; i < robot_traj.getWayPointCount(); ++i)
    {
        ...
        point.time_from_start.nanosec =
            static_cast<uint32_t>((time - static_cast<int>(time)) * 1e9);
...
bool TrajectoryPlanner::apply_time_parameterization(
    robot_trajectory::RobotTrajectory& robot_trajectory, double max_velocity_scaling,
    double max_acceleration_scaling, TimeParameterizationType algorithm)
{
    ...
    case TimeParameterizationType::TOTG:
    {
        ...
        success = totg.computeTimeStamps(robot_trajectory, max_velocity_scaling,
    ...
    case TimeParameterizationType::ITERATIVE_SPLINE:
    {
        ...
        success =
            isp.computeTimeStamps(robot_trajectory, max_velocity_scaling, max_acceleration_scaling);
        break;
    }
    ...
    case TimeParameterizationType::ITERATIVE_PARABOLIC:
    {
        ...
        success = iptp.computeTimeStamps(robot_trajectory, max_velocity_scaling,
                                         max_acceleration_scaling);
        break;
    }
    ...
    default:
        ...
        success = isp_default.computeTimeStamps(robot_trajectory, max_velocity_scaling,
                                                max_acceleration_scaling);
        break;
    }
...
bool TrajectoryPlanner::resample_trajectory(trajectory_msgs::msg::JointTrajectory& trajectory,
