    ...
    : node_(node), heartbeat_rate_hz_(heartbeat_rate_hz)
{
    ...
    // 创建发布器
    planned_path_pub_ =
        node_->create_publisher<std_msgs::msg::String>("/visualization/planned_path", 10);

    ik_status_pub_ = node_->create_publisher<std_msgs::msg::String>("/visualization/ik_status", 10);

    // 心跳发布器 - 仅一个主控心跳，供网页判断 m5_grasp 是否在线
    heartbeat_sealien_payload_grasp_pub_ =
        node_->create_publisher<std_msgs::msg::String>("/heartbeat/sealien_payload_grasp", 10);
    ...
    LOG_NAMED_INFO("web_viz", "  发布话题: /heartbeat/sealien_payload_grasp ({:.1f} Hz)", heartbeat_rate_hz_);
}

void WebVisualizer::start_heartbeat()
{
    if (heartbeat_timer_)
    {
    ...
    auto period = std::chrono::milliseconds(static_cast<int>(1000.0 / heartbeat_rate_hz_));

    heartbeat_timer_ =
        node_->create_wall_timer(period, std::bind(&WebVisualizer::heartbeat_callback, this));
...
void WebVisualizer::heartbeat_callback()
{
    publishHeartbeat();
}
...
void WebVisualizer::publishHeartbeat()
{
    ...
    msg.data = "alive";
    if (heartbeat_sealien_payload_grasp_pub_)
    {
        heartbeat_sealien_payload_grasp_pub_->publish(msg);
    }
}

void WebVisualizer::publishPlannedPath(const std::vector<geometry_msgs::msg::Pose>& waypoints)
{
    if (!planned_path_pub_ || waypoints.empty())
        return;
    ...
    for (size_t i = 0; i < waypoints.size(); ++i)
    {
        if (i > 0)
            ss << ",";
        ss << "{\"x\":" << waypoints[i].position.x << ",\"y\":" << waypoints[i].position.y
...
void WebVisualizer::publish_planned_path_from_trajectory(
    const trajectory_msgs::msg::JointTrajectory& trajectory,
    const moveit::core::RobotStatePtr& robot_state, const std::string& eef_link, size_t max_points)
{

    if (!planned_path_pub_ || !robot_state || trajectory.points.empty())
        return;
    ...
    if (trajectory.points.size() > 1 && (trajectory.points.size() - 1) % step != 0)
    {
        ...
        for (size_t j = 0; j < trajectory.joint_names.size() && j < last_point.positions.size();
             ++j)
        {
...
void WebVisualizer::publish_ik_status(bool reachable, double confidence, const std::string& message)
{
    if (!ik_status_pub_)
        return;
    ...
    std::stringstream ss;
    ss << "{\"reachable\":" << (reachable ? "true" : "false") << ",\"confidence\":" << confidence;
    if (!message.empty())
    {
    ...
    LOG_NAMED_DEBUG("web_viz", "发布IK状态: reachable={}, confidence={:.2f}, msg={}", reachable,
                    confidence, message);
}
...
{
    return heartbeat_sealien_payload_grasp_pub_ ? heartbeat_sealien_payload_grasp_pub_->get_subscription_count() : 0;
}
