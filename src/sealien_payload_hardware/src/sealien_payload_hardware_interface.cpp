#include "sealien_payload_hardware/sealien_payload_hardware_interface.hpp"
...
hardware_interface::CallbackReturn SealienPayloadHardwareInterface::on_init(
  const hardware_interface::HardwareInfo & info)
{
      ...
      if (state_if.name == hardware_interface::HW_IF_POSITION)
      {
        ...
        if (!state_if.initial_value.empty())
        {
          try
          {
            ...
            RCLCPP_INFO_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_CYAN,
              "关节 " << joint.name << " 初始位置: " << init_val << " rad (" 
              << init_val * 180.0 / M_PI << "°)");
          }
          catch (const std::exception & e)
          {
            RCLCPP_WARN_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_YELLOW,
  ...
  {
    RCLCPP_ERROR_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_RED, 
      "无效的IP地址: " << robot_ip_);
  }
  ...
  RCLCPP_INFO_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_GREEN,
...
hardware_interface::CallbackReturn SealienPayloadHardwareInterface::on_configure(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_BLUE, "配置硬件接口...");
  return hardware_interface::CallbackReturn::SUCCESS;
}

std::vector<hardware_interface::StateInterface> SealienPayloadHardwareInterface::export_state_interfaces()
{
...
std::vector<hardware_interface::CommandInterface> SealienPayloadHardwareInterface::export_command_interfaces()
{
  const std::vector<std::string> desired_order = {"Joint1", "Joint2", "Joint3", "Joint4", "JointGL", "JointGR"};
...
hardware_interface::CallbackReturn SealienPayloadHardwareInterface::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_BLUE, "激活硬件接口...");
  
  // 连接到机器人
  if (!connect_to_robot())
  {
    RCLCPP_ERROR_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_RED, "无法连接到机器人");
    return hardware_interface::CallbackReturn::ERROR;
  }

  // 启动通信线程
  stop_thread_ = false;
  comm_thread_ = std::thread(&SealienPayloadHardwareInterface::communication_thread, this);
  ...
  RCLCPP_INFO_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_GREEN COLOR_BOLD, "硬件接口已激活 ✓");
  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn SealienPayloadHardwareInterface::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_YELLOW, "停用硬件接口...");
  ...
  RCLCPP_INFO_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_YELLOW, "硬件接口已停用");
  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::return_type SealienPayloadHardwareInterface::read(
...
hardware_interface::return_type SealienPayloadHardwareInterface::write(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  // 检测命令值是否变化
  std::lock_guard<std::mutex> lock(data_mutex_);
...
bool SealienPayloadHardwareInterface::connect_to_robot()
{
  // 使用UDP socket
  socket_fd_ = socket(AF_INET, SOCK_DGRAM, 0);
  if (socket_fd_ < 0)
  {
    RCLCPP_ERROR_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_RED, "创建UDP socket失败");
    return false;
  }
  ...
  connected_ = true;
  last_successful_comm_ = std::chrono::steady_clock::now();
  RCLCPP_INFO_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_GREEN COLOR_BOLD,
    "UDP socket已创建，目标: " << robot_ip_ << ":" << robot_port_);
  return true;
}

void SealienPayloadHardwareInterface::disconnect_from_robot()
{
  if (socket_fd_ >= 0)
  {
    close(socket_fd_);
    socket_fd_ = -1;
    connected_ = false;
    RCLCPP_INFO_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_YELLOW, "已断开与机器人的连接");
  }
}
...
constexpr const char * EMERGENCY_HOLD_FILE = "/tmp/emergency_hold.txt";  // 急停时 m5_grasp 写入，硬件发“当前反馈”作为目标使机械臂立即停止
constexpr int EMERGENCY_HOLD_VALID_SEC = 120;
}

/** 急停保持：若文件存在且 mtime 在有效期内，则发送当前反馈位置作为目标（机械臂保持不动），否则发 hw_commands_ */
static bool read_emergency_hold_requested()
{
  struct stat st;
  if (stat(EMERGENCY_HOLD_FILE, &st) != 0)
    return false;
  time_t now_sec = time(nullptr);
  return (now_sec - st.st_mtime <= EMERGENCY_HOLD_VALID_SEC);
}

bool SealienPayloadHardwareInterface::read_gripper_direct_file(double & gl, double & gr)
{
  struct stat st;
...
bool SealienPayloadHardwareInterface::send_command(const std::vector<double> & joint_positions)
{
  ...
  if (joint_gl_idx < info_.joints.size() && joint_gl_idx < joint_positions.size())
  {
    ...
    if (!std::isnan(joint_gl_pos) && !std::isinf(joint_gl_pos))
    {
      ...
      // {
      //   RCLCPP_INFO(rclcpp::get_logger("SealienPayloadHardwareInterface"),
...
bool SealienPayloadHardwareInterface::receive_feedback()
{
  if (!connected_ || socket_fd_ < 0)
  {
    return false;
  ...
  if (received == 0)
  {
    RCLCPP_WARN_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_YELLOW, "连接已关闭");
    connected_ = false;
    return false;
  }
...
bool SealienPayloadHardwareInterface::parse_feedback_json(const std::string & json_str)
{
  try
  {
    ...
    if (!root.is_array())
    {
      RCLCPP_WARN_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_YELLOW, "JSON不是数组格式");
      return false;
    }
...
void SealienPayloadHardwareInterface::communication_thread()
{
  RCLCPP_INFO_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_CYAN COLOR_BOLD, "通信线程启动");
  ...
  while (!stop_thread_)
  {
    ...
    if (!connected_)
    {
      ...
      {
        RCLCPP_WARN_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_YELLOW, "尝试重新连接...");
        if (connect_to_robot())
        {
          RCLCPP_INFO_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_GREEN, "重连成功 ✓");
        }
    ...
    else
    {
      ...
      if (should_send)
      {
        ...
        bool use_emergency_hold = read_emergency_hold_requested();
        {
          std::lock_guard<std::mutex> lock(data_mutex_);
          if (use_emergency_hold)
          {
            to_send = feedback_positions_;
          }
          else
          {
        ...
        {
          RCLCPP_WARN_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_YELLOW, "发送命令失败");
        }
      }
    }
  ...
  RCLCPP_INFO_COLOR(rclcpp::get_logger("SealienPayloadHardwareInterface"), COLOR_CYAN, "通信线程结束");
}

}  // namespace sealien_payload_hardware

#include "pluginlib/class_list_macros.hpp"
PLUGINLIB_EXPORT_CLASS(sealien_payload_hardware::SealienPayloadHardwareInterface, hardware_interface::SystemInterface)
